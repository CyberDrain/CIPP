...
export const CippTextFieldWithVariables = ({
  ...
  const handleInputChange = (event) => {
    ...
    const justTypedTrigger =
      ...
      newValue.substring(cursorPos - autocompleteTrigger.length, cursorPos) ===
        autocompleteTrigger
    ...
    if (justTypedTrigger) {
      ...
      const lastTriggerIndex = newValue.lastIndexOf(
        autocompleteTrigger,
        cursorPos - 1
      )
      if (lastTriggerIndex !== -1) {
        const query = newValue.substring(
          lastTriggerIndex + autocompleteTrigger.length,
          cursorPos
        )
        setSearchQuery(query)

        // Close autocomplete when the query no longer resembles an in-progress token.
        const shouldCloseForPercent =
          autocompleteTrigger === '%' &&
          (query.includes(' ') || /[^a-zA-Z0-9_]/.test(query))
        const shouldCloseForMustache =
          autocompleteTrigger === '{{' &&
          (query.includes('\n') || query.includes('}'))
  ...
  const handleVariableSelect = useCallback(
    (variableString) => {
      ...
      const lastTriggerIndex = currentValue.lastIndexOf(
        autocompleteTrigger,
        cursorPos - 1
      )
