...
const restoreSettings = () => {
  ...
  return value
    ? stripPersistedBrandingBlobs(stripServerManagedSettings(value))
    : null
}
...
const storeSettings = (value) => {
  try {
    storage.setItem(
      STORAGE_KEY,
      JSON.stringify(stripPersistedBrandingBlobs(value))
    )
  } catch (err) {
    ...
    try {
      ...
      console.error(
        '[Settings Context] Failed to recover settings storage',
        retryErr
      )
    }
  }
