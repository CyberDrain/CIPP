            ...
            confirmText: (
              <Stack spacing={2}>
                <Typography variant="body2">
                  Impersonate this role? CIPP will reload and behave as if you
                  only hold this role — including its tenant restrictions —
                  until you click Exit in the banner at the top of the page. IP
                  restrictions are not simulated.
                </Typography>
                <Alert severity="warning">
                  This tests a <strong>single role in isolation</strong>, not
                  role combinations. For users holding several roles, custom
                  roles are <strong>restrictive, not additive</strong>: combined
                  with a base role like editor or readonly they can only narrow
                  access, so a real user's effective permissions may differ from
                  what you see here.
                </Alert>
              </Stack>
            ),
  ...
  const offCanvas = {
    children: (data) => {
      ...
      if (data['Permissions'] && Object.keys(data['Permissions']).length > 0) {
        properties.push({
          label: hasRules
            ? 'Effective Permissions (at last save)'
            : 'Permissions',
          value: (
            <Stack spacing={0.5}>
