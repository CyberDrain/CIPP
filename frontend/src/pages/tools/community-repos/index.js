...
const Page = () => {
  ...
  const catalogItems = Array.isArray(catalog.data?.Results)
  ...
  const handleOpenRepo = (repo) => {
    router.push(
      `/tools/community-repos/catalog?Repo=${encodeURIComponent(repo.FullName)}`
    )
  }
  ...
  const handleAddRepo = () => {
    ...
    if (!fullName) {
      setAddRepoError(
        "Enter a repository as 'owner/repo' or paste a GitHub URL."
      )
      return
    }
    ...
    <Box sx={{ flexGrow: 1, py: 4 }}>
      ...
      <Container maxWidth="xl">
        <Stack spacing={2}>
            ...
            <Stack direction="row" spacing={1}>
              ...
              <Button
                ...
                disabled={
                  ...
                  !integrations?.data?.GitHub?.Enabled
                }
              >
                Create Repository
              </Button>
                      ...
                      >
                        <CardContent sx={{ flexGrow: 1 }}>
                          ...
                          >
                            ...
                            <IconButton
                              size="small"
                              onClick={(event) => handleOpenMenu(event, repo)}
                            >
                          ...
                          >
                            {(() => {
                              ...
                              const typeLabels = [
                                ...new Set(
                                  (Array.isArray(repo.TemplateTypes)
                                    ? repo.TemplateTypes
                                    : []
                                  ).map((type) => getTemplateTypeLabel(type))
                                ),
                              ]
                          ...
                          >
                            {catalog.isSuccess ? (
                              <Box
                                sx={{
                                  backgroundColor:
                                    count > 0 ? 'success.main' : 'warning.main',
                                  borderRadius: '50%',
                                  width: 8,
                                  height: 8,
                                }}
                ...
                onClick={() => {
                  ...
                  setBranchValue(
                    menuRepo?.UploadBranch
                      ? {
                          label: menuRepo.UploadBranch,
                          value: menuRepo.UploadBranch,
                        }
                      : null
                  )
                  handleCloseMenu()
            ...
            {menuRepo?.BuiltIn !== true && (
              <MenuItem
                onClick={() => {
                  ...
                  setTypesValue(
                    (Array.isArray(menuRepo?.TemplateTypes)
                      ? menuRepo.TemplateTypes
                      : []
                    ).map((type) => ({
            ...
            <DialogContent>
              <Stack spacing={2} sx={{ mt: 1 }}>
                ...
                <CippAutoComplete
                  ...
                  value={addRepoTypes}
                  onChange={(newValue) =>
                    setAddRepoTypes(Array.isArray(newValue) ? newValue : [])
                  }
                  options={typeOptions}
                  multiple={true}
                  creatable={false}
                ...
                <Typography variant="body2" color="text.secondary">
                  The template type is used to show templates from this
                  repository in the right catalogs. Repositories that use CIPP
                  folder names are detected automatically.
                </Typography>
              </Stack>
          ...
          >
            <DialogTitle>
              Set Upload Branch for {branchDialogRepo?.Name}
            </DialogTitle>
            <DialogContent>
              <Box sx={{ mt: 1 }}>
                <CippAutoComplete
            ...
            <DialogActions>
              <Button
                variant="outlined"
                onClick={() => setBranchDialogRepo(null)}
                startIcon={<Close />}
              >
                Close
              </Button>
          ...
          >
            <DialogTitle>
              Set Template Type for {typesDialogRepo?.Name}
            </DialogTitle>
            <DialogContent>
              <Box sx={{ mt: 1 }}>
                <CippAutoComplete
                  ...
                  value={typesValue}
                  onChange={(newValue) =>
                    setTypesValue(Array.isArray(newValue) ? newValue : [])
                  }
                  options={typeOptions}
                  multiple={true}
                  label="Template Types"
            ...
            <DialogActions>
              <Button
                variant="outlined"
                onClick={() => setTypesDialogRepo(null)}
                startIcon={<Close />}
              >
                Close
              </Button>
          ...
          >
            ...
            <DialogContent>
              <Typography variant="body1">
                Are you sure you want to remove '{deleteDialogRepo?.FullName}'
                as a template source? Templates you have already imported are
                not affected.
              </Typography>
            ...
            <DialogActions>
              <Button
                variant="outlined"
                onClick={() => setDeleteDialogRepo(null)}
                startIcon={<Close />}
              >
                Close
              </Button>
