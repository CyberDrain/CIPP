...
const dialogPropsOf = (dialog) =>
  React.Children.toArray(dialog?.props?.children).find(
    (child) => child?.type === CippApiDialog
  )?.props

const Harness = ({ actions, data = { id: '1' }, queryKeys, onDialogProps }) => {
  const { visibleActions, dispatch, dialog } = useActionsDispatch({
    actions,
    data,
    queryKeys,
  })
...
describe('useActionsDispatch', () => {
  ...
  it('runs a noConfirm customFunction exactly once per tap', async () => {
    ...
    renderWithProviders(
      <Harness
        actions={[{ label: 'Refresh Data', noConfirm: true, customFunction }]}
      />
    )
  ...
  it('does not let the action object override explicit dialog props', async () => {
    ...
    renderWithProviders(
      <Harness
        actions={[
          { label: 'Edit', type: 'POST', url: '/api/Edit', row: 'not-the-row' },
        ]}
        data={{ id: '42' }}
        onDialogProps={props}
      />
  ...
  it('hands a customComponent action to that component instead of a confirm dialog', async () => {
    ...
    const customComponent = vi.fn(() => (
      <div data-testid="custom">custom surface</div>
    ))
    renderWithProviders(
      <Harness actions={[{ label: 'Open', customComponent }]} />
    )
