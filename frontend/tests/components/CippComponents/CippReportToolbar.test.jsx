...
vi.mock(
  '../../../src/components/CippComponents/CippAddTestReportDrawer',
  () => ({
...
describe('CippReportToolbar', () => {
  ...
  it('renders the inline desktop action buttons', () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Refresh test suites' })
    ).toBeInTheDocument()
    expect(
      screen.queryByRole('button', { name: 'Test suite actions' })
  ...
  it('collapses to a sheet trigger + kebab on mobile — no text input, no keyboard', () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Test suite actions' })
    ).toBeInTheDocument()
    expect(
    ...
    expect(
      screen.queryByRole('button', { name: 'Refresh test suites' })
    ).not.toBeInTheDocument()
    // the house pick-one pattern: a trigger, not an autocomplete
    expect(screen.queryByRole('combobox')).not.toBeInTheDocument()
    expect(
      screen.getByRole('button', { name: /switch test suite/i })
    ).toHaveTextContent('Zero Trust Network Access Tests')
  })
    ...
    await waitFor(() =>
      expect(routerState.push).toHaveBeenCalledWith(
        expect.objectContaining({
          query: expect.objectContaining({ reportId: 'custom-1' }),
        }),
        undefined,
        { shallow: true }
      )
  ...
  it('offers all five suite actions in the sheet', async () => {
    ...
    ;[
      ...
      'Delete Suite',
      'Reload suite list',
    ].forEach((label) => expect(sheet.getByText(label)).toBeInTheDocument())
  })
  ...
  it('disables Edit and Delete with a visible reason for a built-in suite', async () => {
    ...
    const sheet = await openActionSheet(user)
    expect(
      sheet.getByText('Built-in test suites cannot be edited')
    ).toBeInTheDocument()
    expect(
      sheet.getByText('Built-in test suites cannot be deleted')
    ).toBeInTheDocument()
    expect(
      sheet.getByText('Edit Suite').closest("[role='button']")
  ...
  it('opens the run-tests dialog and keeps it mounted after the sheet closes', async () => {
    ...
    expect(await screen.findByTestId('api-dialog')).toHaveTextContent(
      'Refresh Test Data'
    )
    await waitFor(() =>
