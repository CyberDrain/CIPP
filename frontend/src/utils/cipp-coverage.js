...
export function classifyRow(r) {
  ...
  if (NON_TERMINAL.includes(r.State) && Number(r.Attempts) > 0)
    return 'Retrying'
...
export function tenantGrid(rows, b) {
  ...
  for (const [name, tRows] of byTenant) {
    ...
    const onlySkipped =
      tRows.length > 0 && tRows.every((r) => r.State === 'Skipped')
...
export function summarize(rows) {
  ...
  if (cols.length) {
    for (const t of tenantWindowGrid(rows, cols))
      gaps += t.states.filter((s) => s === 'Gap').length
  }
  return {
    total,
    processed,
