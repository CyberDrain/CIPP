      ...
      },
      {
        onSuccess: (data) => {
          ...
          setSelectedBackupData(
            validation.isValid && validation.data ? validation.data : null
          )
          setWizardLoading(false)
        },
    ...
    <>
        ...
        infoBar={
          <Stack spacing={2}>
            <Alert severity="info" sx={{ mt: 2 }}>
              Backups are stored in the storage account associated with your
              CIPP instance. You can download or restore specific points in time
              from the list below. Enable automatic backups to have CIPP create
              daily backups using the scheduler.
            </Alert>
            <CippInfoBar
              ...
              data={[
                ...
                },
                {
                  ...
                  data: (
                    <NextBackupRun
                      date={scheduledBackup.data?.[0]?.ScheduledTime}
                    />
                  ),
                },
              ]}
                  ...
                  <>
                    <Stack spacing={2} direction="row">
                      ...
                      {scheduledBackup.isSuccess &&
                        scheduledBackup.data?.[0]?.Name !==
                          'Automated CIPP Backup' && (
                          <Button
                            color="primary"
          ...
          <Card>
            ...
            <CardContent>
              <Alert severity="info" sx={{ mb: 2 }}>
                When enabled, each new backup is also uploaded to the external
                container described by the SAS URL. The SAS URL is stored
                securely in Key Vault. This does not copy existing backups. This
                will continue to push backups to the container without any
                consideration for storage costs, so please monitor your external
                storage usage.
              </Alert>
      ...
      <CippApiDialog
        ...
        api={{
          ...
          data: { Enabled: false },
          confirmText:
            'Are you sure you want to remove the automatic backup schedule?',
          relatedQueryKeys: ['ScheduledBackup'],
        }}
      />
