...
export const CippMessageDeliveryInfo = ({ emailSource }) => {
  const { hops, totalMs, auth } = useMemo(() => {
    ...
    const received = getHeaderValues(lines, 'Received').map(parseHop).reverse()
    ...
    const grab = (key) =>
      authText.match(new RegExp(`\\b${key}=(\\w+)`, 'i'))?.[1] ?? null
    const authResults = {
      SPF: grab('spf'),
      DKIM: grab('dkim'),
      DMARC: grab('dmarc'),
      ...
      <CardContent>
        {authEntries.length > 0 && (
          <Stack
            useFlexGap
            direction="row"
            sx={{ mb: hasHops ? 3 : 0, flexWrap: 'wrap', gap: 1 }}
          >
