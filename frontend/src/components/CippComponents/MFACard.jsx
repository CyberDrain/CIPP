...
export const MFACard = ({ data, isLoading }) => {
  ...
  const processData = () => {
    ...
    enabledUsers.forEach((user) => {
      ...
      const perUserEnabled =
        user.PerUser === 'enforced' || user.PerUser === 'enabled'
    ...
    const registeredPercentage = (
      (registeredUsers / enabledUsers.length) *
      100
    ).toFixed(1)
    const protectedPercentage = (
      ((registeredCA + registeredSD + registeredPerUser) /
        enabledUsers.length) *
      100
    ).toFixed(1)
    ...
    const links = [
      {
        source: 'Enabled users',
        target: 'MFA registered',
    ...
    if (registeredPerUser > 0)
      links.push({
        source: 'MFA registered',
        target: 'Per-user MFA',
        value: registeredPerUser,
      })
    if (registeredNone > 0)
      links.push({
        source: 'MFA registered',
        target: 'No enforcement',
        value: registeredNone,
      })
    ...
    if (notRegisteredPerUser > 0)
      links.push({
        source: 'Not registered',
        target: 'Per-user MFA',
        value: notRegisteredPerUser,
      })
    if (notRegisteredNone > 0)
      links.push({
        source: 'Not registered',
        target: 'No enforcement',
        value: notRegisteredNone,
      })
  ...
  const handleLinkClick = (link) => {
    ...
    if (
      link.source.id === 'Enabled users' &&
      link.target.id === 'MFA registered'
    ) {
      filters = [
        { id: 'AccountEnabled', value: 'Yes' },
        { id: 'MFARegistration', value: 'Yes' },
      ...
      link.source.id === 'MFA registered' &&
      link.target.id === 'CA policy'
    ) {
      filters = [
        { id: 'AccountEnabled', value: 'Yes' },
        { id: 'MFARegistration', value: 'Yes' },
      ...
      link.source.id === 'MFA registered' &&
      link.target.id === 'Security defaults'
    ) {
      filters = [
        { id: 'AccountEnabled', value: 'Yes' },
        { id: 'MFARegistration', value: 'Yes' },
      ...
      link.source.id === 'MFA registered' &&
      link.target.id === 'Per-user MFA'
    ) {
      filters = [{ id: 'AccountEnabled', value: 'Yes' }]
      // Note: Per-user MFA can be "enabled" or "enforced"
    } else if (
      link.source.id === 'MFA registered' &&
      link.target.id === 'No enforcement'
    ) {
      filters = [
        { id: 'AccountEnabled', value: 'Yes' },
        { id: 'MFARegistration', value: 'Yes' },
