...
export const CippTransportRuleDrawer = ({
  ...
  const conditionTypeWatch = useWatch({
    control: formControl.control,
    name: 'conditionType',
  })
  const actionTypeWatch = useWatch({
    control: formControl.control,
    name: 'actionType',
  })
  const exceptionTypeWatch = useWatch({
    control: formControl.control,
    name: 'exceptionType',
  })
  ...
  const processedRuleData = useMemo(() => {
    ...
    const conditionFieldMap = {
      ...
      AnyOfRecipientAddressContainsWords:
        'Any recipient address contains words...',
      AnyOfRecipientAddressMatchesPatterns:
        'Any recipient address matches patterns...',
    }
    ...
    const actionFieldMap = {
      ...
      RouteMessageOutboundConnector:
        'Route the message using the connector named...',
    ...
    Object.keys(actionFieldMap).forEach((field) => {
      ...
      if (
        ...
        activeActions.push({
          value: 'RejectMessage',
          label: actionFieldMap[field],
        })
      } else if (
        field === 'SetHeaderName' &&
        (rule.SetHeaderName || rule.SetHeaderValue)
      ) {
        activeActions.push({ value: 'SetHeader', label: actionFieldMap[field] })
      } else if (
        field === 'ApplyHtmlDisclaimerText' &&
        rule.ApplyHtmlDisclaimerText
      ) {
        activeActions.push({
          value: 'ApplyHtmlDisclaimer',
          label: actionFieldMap[field],
        })
    ...
    Object.keys(conditionFieldMap).forEach((field) => {
      if (rule[field] !== null && rule[field] !== undefined) {
        if (field === 'FromScope' || field === 'SentToScope') {
          ...
          if (Array.isArray(rule[field])) {
            formData[field] = rule[field].map((ip) => ({
              value: ip,
              label: ip,
            }))
          } else {
          ...
          if (Array.isArray(rule[field])) {
            formData[field] = rule[field].map((item) => ({
              value: item,
              label: item,
            }))
          } else {
      ...
      if (field === 'HeaderContainsWords' && rule.HeaderContainsMessageHeader) {
        formData.HeaderContainsWordsMessageHeader =
          rule.HeaderContainsMessageHeader
      }
      if (
        field === 'HeaderMatchesPatterns' &&
        rule.HeaderMatchesMessageHeader
      ) {
        formData.HeaderMatchesPatternsMessageHeader =
          rule.HeaderMatchesMessageHeader
      }
    })
    ...
    if (rule.RejectMessageEnhancedStatusCode)
      formData.RejectMessageEnhancedStatusCode =
        rule.RejectMessageEnhancedStatusCode
    ...
    if (rule.IncidentReportContent) {
      const incidentReportContentValues = Array.isArray(
        ...
        : rule.IncidentReportContent.split(',')
            .map((item) => item.trim())
            .filter(Boolean)
      formData.IncidentReportContent = incidentReportContentValues.map(
    ...
    Object.keys(actionFieldMap).forEach((field) => {
      if (
        ...
        if (field === 'SetSCL' && rule[field] !== null) {
          formData[field] = {
            value: rule[field].toString(),
            label: rule[field].toString(),
          }
    ...
    Object.keys(conditionFieldMap).forEach((field) => {
        ...
        if (field === 'FromScope' || field === 'SentToScope') {
          ...
          formData[exceptionField] =
            rule[exceptionField] !== null
              ? {
                  value: rule[exceptionField].toString(),
                  label: rule[exceptionField].toString(),
                }
              : undefined
        } else if (field === 'SenderIpRanges') {
          // Transform array of IP strings to autocomplete format
          if (Array.isArray(rule[exceptionField])) {
            formData[exceptionField] = rule[exceptionField].map((ip) => ({
              value: ip,
              label: ip,
            }))
          } else {
      ...
      if (
        field === 'HeaderContainsWords' &&
        rule[`ExceptIfHeaderContainsMessageHeader`]
      ) {
        formData.ExceptIfHeaderContainsWordsMessageHeader =
          rule.ExceptIfHeaderContainsMessageHeader
      }
      if (
        field === 'HeaderMatchesPatterns' &&
  ...
  useEffect(() => {
    if (
      !drawerVisible ||
      isEditMode ||
      !Array.isArray(allRulesInfo.data?.Results)
    ) {
      return
    }

    const priorities = allRulesInfo.data.Results.map((rule) =>
  ...
  const customDataFormatter = useCallback(
    (values) => {
      ...
      const apiData = {
        ...
        SetAuditSeverity:
          values.SetAuditSeverity?.value || values.SetAuditSeverity,
        SenderAddressLocation:
          values.SenderAddressLocation?.value || values.SenderAddressLocation,
      ...
      conditionTypes.forEach((condition) => {
        ...
        if (
          conditionValue === 'HeaderContainsWords' &&
          values.HeaderContainsWordsMessageHeader
        ) {
          apiData.HeaderContainsMessageHeader =
            values.HeaderContainsWordsMessageHeader
        ...
        if (
          ...
          apiData.HeaderMatchesMessageHeader =
            values.HeaderMatchesPatternsMessageHeader
      ...
      actionTypes.forEach((action) => {
        ...
        if (actionValue === 'RejectMessage') {
          ...
          if (values.RejectMessageEnhancedStatusCode) {
            apiData.RejectMessageEnhancedStatusCode =
              values.RejectMessageEnhancedStatusCode
          }
        } else if (actionValue === 'SetHeader') {
          if (values.SetHeaderName) apiData.SetHeaderName = values.SetHeaderName
          if (values.SetHeaderValue)
            apiData.SetHeaderValue = values.SetHeaderValue
          ...
          if (values.ApplyHtmlDisclaimerFallbackAction) {
            ...
            apiData.ApplyHtmlDisclaimerFallbackAction =
          ...
          if (values.IncidentReportContent !== undefined) {
            ...
            const incidentReportValues = Array.isArray(fieldValue)
              ...
              : [fieldValue]
            apiData.IncidentReportContent = incidentReportValues
              .filter(Boolean)
              .join(',')
          }
      ...
      exceptionTypes.forEach((exception) => {
        ...
        if (
          ...
          apiData.ExceptIfHeaderMatchesMessageHeader =
            values.ExceptIfHeaderMatchesPatternsMessageHeader
          apiData.ExceptIfHeaderMatchesPatterns =
            values.ExceptIfHeaderMatchesPatterns
        }
      })
  ...
  const conditionOptions = [
    ...
    {
      value: 'SubjectOrBodyContainsWords',
      label: 'Subject or body contains words...',
    },
    {
      value: 'SubjectOrBodyMatchesPatterns',
      label: 'Subject or body matches patterns...',
    },
    {
      value: 'FromAddressContainsWords',
      label: 'Sender address contains words...',
    },
    {
      value: 'FromAddressMatchesPatterns',
      label: 'Sender address matches patterns...',
    },
    {
      value: 'AttachmentContainsWords',
      label: 'Attachment content contains words...',
    },
    {
      value: 'AttachmentMatchesPatterns',
      label: 'Attachment content matches patterns...',
    },
    {
      value: 'AttachmentNameMatchesPatterns',
      label: 'Attachment name matches patterns...',
    },
    {
      value: 'AttachmentPropertyContainsWords',
      label: 'Attachment properties contain words...',
    },
    {
      value: 'AttachmentExtensionMatchesWords',
      label: 'Attachment extension is...',
    },
    {
      value: 'AttachmentHasExecutableContent',
      label: 'Attachment has executable content',
    },
    {
      value: 'AttachmentIsPasswordProtected',
      label: 'Attachment is password protected',
    },
    {
      value: 'AttachmentIsUnsupported',
      label: 'Attachment type is unsupported',
    },
    {
      value: 'AttachmentProcessingLimitExceeded',
      label: 'Attachment processing limit exceeded',
    },
    {
      value: 'AttachmentSizeOver',
      label: 'Attachment size is greater than...',
    },
    ...
    {
      value: 'SenderIpRanges',
      label: 'Sender IP address belongs to any of these ranges...',
    },
    { value: 'HeaderContainsWords', label: 'Message header contains words...' },
    {
      value: 'HeaderMatchesPatterns',
      label: 'Message header matches patterns...',
    },
    { value: 'AnyOfToHeader', label: 'Any To header contains...' },
    {
      value: 'AnyOfToHeaderMemberOf',
      label: 'Any To header is a member of...',
    },
    { value: 'AnyOfCcHeader', label: 'Any Cc header contains...' },
    {
      value: 'AnyOfCcHeaderMemberOf',
      label: 'Any Cc header is a member of...',
    },
    { value: 'AnyOfToCcHeader', label: 'Any To or Cc header contains...' },
    {
      value: 'AnyOfToCcHeaderMemberOf',
      label: 'Any To or Cc header is a member of...',
    },
    {
      value: 'RecipientAddressContainsWords',
      label: 'Recipient address contains words...',
    },
    {
      value: 'RecipientAddressMatchesPatterns',
      label: 'Recipient address matches patterns...',
    },
    {
  ...
  const actionOptions = [
    {
      value: 'DeleteMessage',
      label: 'Delete the message without notifying anyone',
    },
    ...
    {
      value: 'ModerateMessageByUser',
      label: 'Forward the message for approval to...',
    },
    {
    ...
    {
      value: 'GenerateIncidentReport',
      label: 'Generate incident report and send to...',
    },
    {
      value: 'GenerateNotification',
      label: 'Notify the sender with a message...',
    },
          ...
          <Grid size={12} key={actionValue}>
            <CippFormComponent
              ...
              api={{
                ...
                dataFilter: (options) =>
                  options.filter(
                    (option) => option.rawData?.cippconnectortype === 'outbound'
                  ),
              }}
            />
          </Grid>
  ...
  return (
    <>
      ...
      <CippOffCanvas
        title={
          isEditMode ? `Edit Rule: ${rule?.Name || ''}` : 'New Transport Rule'
        }
      ...
      >
        <Grid container spacing={2}>
          ...
          {applyToAllMessagesWatch && (
            <Grid size={12}>
              <Alert severity="info">
                This rule will apply to ALL inbound and outbound messages for
            ...
            <>
              <Grid size={12}>
                <Alert severity="info" sx={{ mb: 2 }}>
                  Select one or more conditions to target specific messages. If
                  you want this rule to apply to all messages, enable "Apply to
                  all messages" above.
                </Alert>
              </Grid>
              ...
              {selectedConditions.map((condition) =>
                renderConditionField(condition)
              )}
            </>
          )}
          ...
          {selectedExceptions.map((exception) =>
            renderExceptionField(exception)
          )}
