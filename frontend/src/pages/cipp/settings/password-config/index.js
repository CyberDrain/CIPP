...
function normalizeConfigForBackend(config) {
  return {
    ...
    charCount: String(
      parseInt(config.charCount, 10) || DEFAULT_VALUES.CHAR_COUNT
    ),
    ...
    specialCharSet: String(
      config.specialCharSet || DEFAULT_VALUES.SPECIAL_CHAR_SET
    ),
    wordCount: String(
      parseInt(config.wordCount, 10) || DEFAULT_VALUES.WORD_COUNT
    ),
    separator:
      config.separator !== undefined && config.separator !== null
        ? String(config.separator)
        : DEFAULT_VALUES.SEPARATOR,
...
const Page = () => {
  ...
  const passwordSave = ApiPostCall({
    datafromUrl: true,
  ...
  useEffect(() => {
    if (passwordSetting.isSuccess && passwordSetting.data) {
      ...
      setConfig({
        ...
        charCount: String(
          parseInt(r.charCount, 10) || DEFAULT_CONFIG.charCount
        ),
        includeUppercase: toBool(
          r.includeUppercase,
          DEFAULT_CONFIG.includeUppercase
        ),
        includeLowercase: toBool(
          r.includeLowercase,
          DEFAULT_CONFIG.includeLowercase
        ),
        includeDigits: toBool(r.includeDigits, DEFAULT_CONFIG.includeDigits),
        includeSpecialChars: toBool(
          r.includeSpecialChars,
          DEFAULT_CONFIG.includeSpecialChars
        ),
        specialCharSet: r.specialCharSet || DEFAULT_CONFIG.specialCharSet,
        wordCount: String(
          parseInt(r.wordCount, 10) || DEFAULT_CONFIG.wordCount
        ),
        separator:
          r.separator !== undefined ? r.separator : DEFAULT_CONFIG.separator,
        capitalizeWords: toBool(
          r.capitalizeWords,
          DEFAULT_CONFIG.capitalizeWords
        ),
        appendNumber: toBool(r.appendNumber, DEFAULT_CONFIG.appendNumber),
        appendSpecialChar: toBool(
          r.appendSpecialChar,
          DEFAULT_CONFIG.appendSpecialChar
        ),
      })
    }
                  ...
                  >
                    ...
                    <Stack direction="row" alignItems="center" spacing={2}>
                      <ToggleButtonGroup
                        ...
                        color="primary"
                      >
                        <ToggleButton value={PASSWORD_TYPES.CLASSIC}>
                          Classic
                        </ToggleButton>
                        <ToggleButton value={PASSWORD_TYPES.PASSPHRASE}>
                          Passphrase
                        </ToggleButton>
                      </ToggleButtonGroup>
                      <Button
                        ...
                        <Grid size={{ sm: 6, xs: 12 }}>
                          <FormControlLabel
                            control={
                              <Switch
                                size="small"
                                checked={config.appendSpecialChar}
                                onChange={(e) =>
