...
const replaceGuidsAndUpnsInString = (
  str,
  guidMapping,
  upnMapping,
  isLoadingGuids
...
export const useGuidResolver = (manualTenant = null) => {
  ...
  const retryApiCallWithBackoff = useCallback(
    (apiCall, url, data, retryDelay = null) => {
      ...
      rateLimitBackoffRef.current = Math.min(
        rateLimitBackoffRef.current * 1.5,
        10000
      )
    },
    []
  ...
  const directoryObjectsMutation = ApiPostCall({
    ...
    onResult: (data) => {
      ...
      if (data && Array.isArray(data.value)) {
        ...
        data.value.forEach((item) => {
          if (item.id) {
            ...
            if (item.displayName || item.userPrincipalName || item.mail) {
              newDisplayMapping[item.id] =
                item.displayName || item.userPrincipalName || item.mail
            }
        ...
        const notReturned = [...processedGuids].filter(
        ...
        if (notReturned.length > 0) {
          ...
          const now = Date.now()
          if (
            !rateLimitTimeoutRef.current &&
            now - lastPartnerRequestTimeRef.current >= 2000
          ) {
  ...
  const partnerDirectoryObjectsMutation = ApiPostCall({
    ...
    onResult: (data) => {
      ...
      if (data && data.statusCode === 429) {
        console.log(
          'Rate limit hit on partner directory objects lookup, retrying...'
        )
        ...
        const currentTenantEntries = [
          ...pendingPartnerGuidsRef.current.entries(),
        ]
        ...
        data.value.forEach((item) => {
          if (item.id) {
            ...
            if (item.userPrincipalName || item.mail || item.displayName) {
              newDisplayMapping[item.id] =
                item.userPrincipalName || item.mail || item.displayName
            }
    ...
    (objectToScan) => {
      ...
      if (guidsSet.size > 0) {
        ...
        if (notResolvedGuids.length > 0) {
          ...
          const allPendingGuids = [
            ...new Set([...pendingGuidsRef.current, ...notResolvedGuids]),
          ]
          ...
          if (
            !rateLimitTimeoutRef.current &&
            now - lastRequestTimeRef.current >= 2000
          ) {
  ...
  const replaceGuidsAndUpnsInStringMemoized = useCallback(
    (str) =>
      replaceGuidsAndUpnsInString(str, guidMapping, upnMapping, isLoadingGuids),
