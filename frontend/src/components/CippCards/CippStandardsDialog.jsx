...
export const CippStandardsDialog = ({
  open,
  onClose,
  standardsData,
  currentTenant,
  ...
  Object.entries(combinedStandards).forEach(([standardKey, standardConfig]) => {
    const standardInfo = getStandards().find(
      (s) => s.name === `standards.${standardKey}`
    )
    if (standardInfo) {
        ...
        <Stack spacing={2}>
          <Box>
            <Typography variant="body2" color="text.secondary" gutterBottom>
              Showing standards configuration for tenant:{' '}
              <strong>{currentTenant}</strong>
            </Typography>
            <Typography variant="body2" color="text.secondary">
              Total templates applied:{' '}
              <strong>{applicableTemplates.length}</strong> | Total standards:{' '}
              <strong>{totalStandardsCount}</strong>
            </Typography>
          </Box>

          {Object.entries(standardsByCategory).map(
            ([category, categoryStandards], idx) => {
              // Calculate the actual count of standards in this category (counting template instances)
              const categoryCount = categoryStandards.reduce(
                (count, { config }) => {
          ...
          {Object.keys(standardsByCategory).length === 0 && (
            <Box textAlign="center" py={4}>
              ...
              <Typography variant="body2" color="text.secondary">
                Standards templates may not be applied to this tenant or no
                standards are currently active.
              </Typography>
            </Box>
          )}
        </Stack>
