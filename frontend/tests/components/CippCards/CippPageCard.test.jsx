...
describe('CippPageCard title vs the mobile tab picker', () => {
  ...
  it('stands its title down when the picker already says it', () => {
    ...
    expect(
      screen.getByRole('button', { name: /CIPP Roles switch view/i })
    ).toBeInTheDocument()
    expect(
      screen.queryByRole('heading', { level: 4, name: 'CIPP Roles' })
  ...
  it('keeps its title on desktop, where tabs look like navigation', () => {
    ...
    expect(
      screen.getByRole('heading', { level: 4, name: 'CIPP Roles' })
    ).toBeInTheDocument()
  })
})
