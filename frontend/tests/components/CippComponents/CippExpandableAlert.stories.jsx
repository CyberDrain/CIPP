...
export const ClampsLongMessagesOnAPhone = {
  render: () => (
    <CippExpandableAlert severity="info">{LONG_TEXT}</CippExpandableAlert>
  ),
  play: async ({ canvasElement, step }) => {
    ...
    await step('expanding shows everything and offers Show less', async () => {
      ...
      const message = canvas.getByText(/Custom roles/, { exact: false })
      await waitFor(() => {
        expect(message.scrollHeight).toBeLessThanOrEqual(
          message.clientHeight + 1
        )
        expect(
...
export const LeavesShortMessagesAlone = {
  render: () => (
    <CippExpandableAlert severity="info">{SHORT_TEXT}</CippExpandableAlert>
  ),
  play: async ({ canvasElement, step }) => {
    const onAPhone = await shrinkToPhoneViewport()
    if (!onAPhone) return
