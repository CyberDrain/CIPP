...
describe('HeaderedTabbedLayout mobile header', () => {
  ...
  it('puts the tab picker in the title row on mobile, and tabs on desktop', async () => {
    ...
    const sheet = within(
      (await screen.findByText('Views')).closest('.MuiDrawer-paper')
    )
    ...
    await waitFor(() =>
      expect(
        screen.getByRole('tab', { name: /Manage Drift/ })
      ).toBeInTheDocument()
    )
  })
  ...
  it("opens the action's overlay from the sheet and leaves it open", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText('Reset Password')).not.toBeVisible()
    )
