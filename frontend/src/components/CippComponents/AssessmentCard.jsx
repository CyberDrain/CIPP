...
export const AssessmentCard = ({ data, isLoading, title, description }) => {
  ...
  const identityInformational =
    data?.TestResultSummary?.IdentityInformational || 0
  const identityNeedsAttention =
    data?.TestResultSummary?.IdentityNeedsAttention || 0
  ...
  const devicesInformational =
    data?.TestResultSummary?.DevicesInformational || 0
  const devicesNeedsAttention =
    data?.TestResultSummary?.DevicesNeedsAttention || 0
  ...
  const customNeedsAttention =
    data?.TestResultSummary?.CustomNeedsAttention || 0
  ...
  const overallDenominator =
    overallCategory.total > 0 ? overallCategory.total : overallTotalFromValues
  const overallPassWidth =
    overallDenominator > 0
      ? (overallCategory.passed / overallDenominator) * 100
      : 0
  const overallFailWidth =
    overallDenominator > 0
      ? (overallCategory.failed / overallDenominator) * 100
      : 0
  const overallSkipWidth =
    overallDenominator > 0
      ? (overallCategory.skipped / overallDenominator) * 100
      : 0
  const overallInfoWidth =
    overallDenominator > 0
      ? (overallCategory.informational / overallDenominator) * 100
      : 0
  const overallNeedsAttentionWidth =
    overallDenominator > 0
      ? (overallCategory.needsAttention / overallDenominator) * 100
      : 0
  ...
  const descriptionText =
    description || 'No description available for the selected test suite.'
                ...
                {testCategories.length > 0 ? (
                  ...
                  >
                    {testCategories.map((category) => {
                      ...
                      const denominator =
                        category.total > 0 ? category.total : totalFromValues
                      const passWidth =
                        denominator > 0
                          ? (category.passed / denominator) * 100
                          : 0
                      const failWidth =
                        denominator > 0
                          ? (category.failed / denominator) * 100
                          : 0
                      const skipWidth =
                        denominator > 0
                          ? (category.skipped / denominator) * 100
                          : 0
                      const infoWidth =
                        denominator > 0
                          ? (category.informational / denominator) * 100
                          : 0
                      const needsAttentionWidth =
                        denominator > 0
                          ? (category.needsAttention / denominator) * 100
                          : 0
                      const statusItems = [
                        {
                          key: 'pass',
              ...
              overallCategory.show && (
                <Box sx={{ textAlign: 'left', width: '100%' }}>
                  ...
                  >
                    <Box
                      sx={{
                        width: `${overallPassWidth}%`,
                        bgcolor: 'success.main',
                      }}
                    />
                    <Box
                      sx={{
                        width: `${overallFailWidth}%`,
                        bgcolor: 'error.main',
                      }}
                    />
                    <Box
                      sx={{
                        width: `${overallSkipWidth}%`,
                        bgcolor: 'warning.main',
                      }}
                    />
                    <Box
                      sx={{
                        width: `${overallInfoWidth}%`,
                        bgcolor: 'info.main',
                      }}
                    />
                    <Box
                      sx={{
                        width: `${overallNeedsAttentionWidth}%`,
                        bgcolor: 'text.primary',
                      }}
                    />
                  </Box>
                  <Typography
                    variant="caption"
                    sx={{ display: 'block', lineHeight: 1.25, mt: 0.25 }}
                  >
