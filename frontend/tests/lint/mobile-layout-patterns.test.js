...
const dashboardFiles = files.filter((file) =>
  rel(file).startsWith(path.join('pages', 'dashboardv2'))
)
...
describe('mobile layout patterns', () => {
  ...
  it('declares no Grid column split that survives a phone', () => {
    const offenders = files.flatMap((file) =>
      gridOffenders(fs.readFileSync(file, 'utf8')).map(
        (offender) => `${rel(file)}:${offender}`
      )
    )
    expect(
      offenders,
      `Use size={{ xs: 12, sm|md: N }} instead:\n${offenders.join('\n')}`
    ).toEqual([])
  })
  ...
  it('takes a marked split at its word', () => {
    ...
    expect(
      gridOffenders(`      // two-up by design: ${MARKER}\n${split}`)
    ).toEqual([])
  ...
  it('reads a percent split only as a fixed string width', () => {
    ...
    expect(
      percentSplitOffenders(`      <Box width={{ xs: "100%", xl: "30%" }}>\n`)
    ).toEqual([])
    expect(percentSplitOffenders(`      <Skeleton width="80%" />\n`)).toEqual(
      []
    )
    expect(
      percentSplitOffenders(`      // ${MARKER}\n      <Box width="30%">\n`)
    ).toEqual([])
  })
  ...
  it('reads a pinned height only as a bare number', () => {
    ...
    expect(
      pinnedHeightOffenders(
        `      <Box sx={{ height: { xs: 'auto', lg: 450 } }}>\n`
      )
    ).toEqual([])
    expect(
      pinnedHeightOffenders(`      <Box sx={{ minHeight: 450 }}>\n`)
    ).toEqual([])
    expect(
      pinnedHeightOffenders(`      <Box sx={{ height: '100%' }}>\n`)
    ).toEqual([])
    expect(
      pinnedHeightOffenders(
        `      // ${MARKER}\n      <Box sx={{ height: 450 }}>\n`
  ...
  it('keys layout chrome off the shared breakpoint hook, not its own media query', () => {
    ...
    for (const name of ['index.js', 'top-nav.js']) {
      const source = stripComments(
        fs.readFileSync(path.join(SRC, 'layouts', name), 'utf8')
      )
      source.split('\n').forEach((line, i) => {
