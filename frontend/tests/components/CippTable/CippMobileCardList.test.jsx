  ...
  it('keeps Result and Risk together in the chips row, and labels the mute one', async () => {
    renderWithProviders(
      <CippDataTable
        ...
        data={[
          {
            TestName: 'Tenant has M365 Copilot prerequisites',
            Result: 'Passed',
            Risk: 'High',
          },
        ]}
    ...
    await waitFor(() =>
      expect(
        screen.getByText('Tenant has M365 Copilot prerequisites')
      ).toBeInTheDocument()
    )
    ...
    expect(high.closest('.MuiStack-root')).toBe(
      passed.closest('.MuiStack-root')
    )
...
describe('CippMobileCardList table view toggle', () => {
  ...
  it('opens the table view and the way back restores cards, never touching settings', async () => {
    ...
    expect(
      screen.queryByTestId('cipp-mobile-card-list')
  ...
  it('the toggled table keeps every configured column visible', async () => {
    ...
    const checkbox = (name) =>
      within(menu.getByRole('menuitem', { name })).getByRole('checkbox')
    for (const name of [
      'Display Name',
      'User Principal Name',
      'Account Enabled',
  ...
  it('puts the table utilities above the Fields shown list, not below it', async () => {
    ...
    expect(
      refresh.compareDocumentPosition(fields) & Node.DOCUMENT_POSITION_FOLLOWING
    ).toBe(4)
    expect(
  ...
  it('an explicit viewMode prop hides the toggle button', async () => {
    ...
    expect(
      screen.queryByRole('button', { name: 'Toggle table view' })
    ).not.toBeInTheDocument()
  })
  ...
  it('an applied preset and its badge survive a flip to table and back', async () => {
    ...
    await waitFor(() => {
      expect(
        within(screen.getByRole('button', { name: 'Table options' })).getByText(
          '1'
        )
      ).toBeInTheDocument()
    })
    ...
    const chip = within(reopened)
      .getByText('IT department')
...
describe('CippMobileCardList table-view page actions FAB', () => {
  ...
  it('desktop viewport keeps cardButton in the header, no FAB', async () => {
    ...
    expect(
      screen.queryByRole('button', { name: 'Page actions' })
    ).not.toBeInTheDocument()
  })
})
...
describe('CippMobileCardList table-view header mounts as the bulk-actions portal target', () => {
  ...
  it('desktop + hideTitle + cardButton: header mounts with cardButton in it, same as before', async () => {
    ...
    expect(
      within(header).getByRole('button', { name: 'Add user' })
    ).toBeInTheDocument()
  })
})
...
describe('CippMobileCardList data source controls', () => {
  ...
  it('renders in the Table options sheet, not in the page actions FAB', async () => {
    ...
    renderCards(
      {},
      {
        dataSourceControls: <span>Live badge</span>,
    ...
    await waitFor(() =>
      expect(
        within(fabSheet).getByRole('button', { name: 'Add user' })
      ).toBeInTheDocument()
    )
