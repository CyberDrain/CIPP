...
const siteTypeIcon = (siteType) =>
  siteType === 'teams' ? TeamsIcon : SharePointIcon
...
export const getSiteLanguageOption = (value) => {
  ...
  return (
    SITE_LANGUAGE_OPTIONS.find((option) => option.value === resolved) ??
    SITE_LANGUAGE_OPTIONS[0]
  )
}
...
const LibraryRow = ({
  ...
  return (
    ...
    >
      <Tooltip
        title={missingName ? 'Library name required' : 'Document library'}
      >
        <Folder
          fontSize="small"
          sx={{ color: missingName ? 'error.main' : 'text.secondary' }}
        />
      </Tooltip>
      <Controller
        name={`${name}.name`}
      ...
      {permCount > 0 && (
        <Tooltip
          title={`${permCount} unique permission${permCount > 1 ? 's' : ''}`}
        >
      ...
      >
        ...
        >
          ...
          <ListItemText>
            {permCount > 0 ? 'Edit Permissions' : 'Add Permissions'}
          </ListItemText>
        </MenuItem>
        ...
        <Stack spacing={1}>
          ...
          {overrideActive ? (
            <Typography variant="body2" color="text.secondary">
              Template override is active — all sites use the template site type
              at deploy.
            </Typography>
          ) : (
            <Typography variant="body2" color="text.secondary">
              Choose whether this entry provisions a SharePoint site or a
              Microsoft Team.
            </Typography>
          )}
        </Stack>
...
const SiteLanguageDialog = ({ formControl, name, onClose }) => {
  ...
  useEffect(() => {
    ...
    formControl.setValue(
      fieldName,
      getSiteLanguageOption(formControl.getValues(fieldName)),
      {
        shouldDirty: false,
        shouldTouch: false,
      ...
      <DialogContent dividers>
        {name && (
          <Stack spacing={1}>
            ...
            />
            <Typography variant="body2" color="text.secondary">
              Tenant default follows each target tenant&apos;s SharePoint root
              site language (the SPO default for new sites) at deploy. A
              specific language is applied when the SharePoint site is created.
            </Typography>
          </Stack>
        )}
      </DialogContent>
...
const CreateAsDialog = ({ formControl, name, onClose }) => {
  useEffect(() => {
    ...
    formControl.setValue(
      fieldName,
      resolveCreateAs(formControl.getValues(fieldName)),
      {
        shouldDirty: false,
        shouldTouch: false,
        shouldValidate: false,
      ...
      <DialogContent dividers>
        {name && (
          <Stack spacing={1}>
            ...
            />
            <Typography variant="body2" color="text.secondary">
              Team site is a collaboration workspace. Communication site is for
              publishing (intranet, news). Only applies when this card deploys
...
const SiteTemplateCard = ({
  ...
  const libraries = useWatch({
    control: formControl.control,
    name: `${name}.libraries`,
  })
  const cardSiteType = useWatch({
    control: formControl.control,
    name: `${name}.siteType`,
  })
  const createAs = useWatch({
    control: formControl.control,
    name: `${name}.createAs`,
  })
  const overrideSiteType = useWatch({
    control: formControl.control,
    name: 'overrideSiteType',
  })
  const templateSiteType = useWatch({
    control: formControl.control,
    name: 'siteType',
  })
  const permCount = Array.isArray(permissions) ? permissions.length : 0
  ...
  const effectiveSiteType = resolveSiteType(
    overrideActive ? templateSiteType : cardSiteType
  )
  ...
  const incompleteLibraries = (libraries || []).some(
    (lib) => !lib?.name?.trim()
  )
  const cardBlocksSave =
    missingDisplayName || missingRootPerms || incompleteLibraries
        ...
        >
          {fields.map((field, libIndex) => (
            <LibraryRow
              ...
              onConfigurePermissions={() =>
                onConfigurePermissions(
                  `${name}.libraries.${libIndex}.permissions`,
                  'Library'
                )
              }
            />
...
export const CippSharePointTemplateQuickStats = ({ formControl, sx }) => {
  const siteTemplates =
    useWatch({ control: formControl.control, name: 'siteTemplates' }) || []
  const overrideSiteType = useWatch({
    control: formControl.control,
    name: 'overrideSiteType',
  })
  ...
  const libraryCount = siteTemplates.reduce(
    (total, site) =>
      total + (Array.isArray(site?.libraries) ? site.libraries.length : 0),
    0
  )
  const permissionCount = siteTemplates.reduce((total, site) => {
    const sitePerms = Array.isArray(site?.permissions)
    ...
    const libPerms = Array.isArray(site?.libraries)
      ? site.libraries.reduce(
          (sum, lib) =>
            sum +
            (Array.isArray(lib?.permissions) ? lib.permissions.length : 0),
          0
        )
      : 0
...
const PermissionDialog = ({ formControl, target, onClose }) => (
  <Dialog open={Boolean(target)} onClose={onClose} fullWidth maxWidth="sm">
    <DialogTitle>
      {target?.title === 'Library' ? 'Library Permissions' : 'Site Permissions'}
    </DialogTitle>
    <DialogContent dividers>
      {target && (
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="xs">
      ...
      <DialogContent dividers>
        <Stack spacing={1.5}>
          <Typography variant="body2" color="text.secondary">
            Force every site in this template to deploy as the same type,
            ignoring each card&apos;s own site type.
          </Typography>
          <CippFormComponent
            type="switch"
...
export const CippSharePointTemplateBuilder = ({ formControl }) => {
  ...
  const handleConfigurePermissions = (name, title) =>
    setPermTarget({ name, title })
    ...
    <Box>
      ...
      >
        <Box sx={{ minWidth: 0 }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Each site template provisions a SharePoint site or Microsoft Team
            and its document libraries.
          </Typography>
          {overrideActive && (
            <Typography
              variant="caption"
            ...
            >
              ...
              {resolveSiteType(templateSiteType) === 'teams'
                ? 'Microsoft Teams'
                : 'SharePoint sites'}
              .
            </Typography>
            ...
            onConfigurePermissions={(target, title) =>
              target
                ...
                : handleConfigurePermissions(
                    `siteTemplates.${index}.permissions`,
                    'Site'
                  )
            }
          />
