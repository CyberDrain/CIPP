...
const CippIntegrationSettings = ({ children }) => {
  ...
  const buildApi = (setting) => {
    ...
    const paramName =
      setting.dependsOnParam ?? setting.dependsOn.split('.').pop()
    return {
      ...setting.api,
  ...
  const dependentValues = formControl.watch(
    dependentSettings.map((setting) => setting.name)
  )
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const current = parentNames.map((_, index) =>
      JSON.stringify(parentValues?.[index] ?? null)
    )
    ...
    parentNames.forEach((parentName, index) => {
      ...
      dependentSettings
        ...
        .forEach((setting) =>
          formControl.setValue(setting.name, null, { shouldDirty: true })
        )
    })
    previousParentValues.current = current
              ...
              <React.Fragment key={index}>
                {setting?.condition ? (
                  ...
                  >
                    <Grid
                      size={{
                        xs: 12,
                        md:
                          setting.fullRow || setting.type === 'switch' ? 12 : 6,
                      }}
                    >
                      <Box sx={{ p: 1 }}>
                        <CippFormComponent
                  ...
                  <Grid
                    size={{
                      xs: 12,
                      md: setting.fullRow || setting.type === 'switch' ? 12 : 6,
                    }}
                  >
                    <Box sx={{ p: 1 }}>
                      <CippFormComponent
