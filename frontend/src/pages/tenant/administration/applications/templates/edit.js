...
const Page = () => {
  ...
  const { data: templateData, isLoading } = ApiGetCall({
    url: template
      ? `/api/ExecAppApprovalTemplate?Action=Get&TemplateId=${template}`
      : null,
    queryKey: template
      ? ['ExecAppApprovalTemplateList', template, refetchKey]
      : null,
    enabled: !!template,
  })
      ...
      <CippPageCard hideBackButton={false} title="Template Not Found">
        <CardContent>
          <Alert severity="error" sx={{ mb: 2 }}>
            The requested app approval template does not exist or was not
            specified. Please select a valid template from the list.
          </Alert>
          <Button
            variant="contained"
            component={Link}
