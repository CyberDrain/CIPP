...
const SSO_DOCS_URL =
  'https://docs.cipp.app/user-documentation/cipp/advanced/authentication/sso'
...
const ssoAppPermissions = [
  {
    name: 'openid',
    reason:
      'Signs the user in and issues an ID token. The base OpenID Connect scope.',
  },
  {
    name: 'profile',
    reason:
...
const samPermissionsUsed = [
  {
    ...
    reason:
      'Creates the CIPP-SSO app registration, its service principal and its client secret.',
  },
  {
...
export const CippSSOSettings = () => {
  ...
  const canRepair =
    data?.canRepair ?? (hasAppId && repairableStatuses.has(statusKey))
  const isProvisioned =
  ...
  return (
    <CippButtonCard
      title="SSO App Registration"
      isFetching={ssoStatus.isFetching}
    >
      <CardContent>
        {ssoStatus.isLoading ? (
          <Stack spacing={2}>
          ...
          <Stack spacing={3}>
              ...
              <AccordionDetails sx={{ px: 0 }}>
                <Stack spacing={2}>
                  <Typography variant="body2">
                    CIPP signs users in through an app registration called{' '}
                    <strong>CIPP-SSO</strong> in your own partner tenant, which
                    puts CIPP sign-in under your Conditional Access policies,
                    MFA requirements and session controls. The app only proves
                    who you are — it has no access to any data in your tenant.
                    Everything CIPP does against Microsoft 365 still runs
                    through the CIPP-SAM app and your GDAP relationships.
                  </Typography>

                  <Alert severity="info">
                    You do <strong>not</strong> need Entra ID Global
                    Administrator to run this setup, and there is no separate
                    enterprise app for anyone to approve. CIPP creates the app
                    itself using permissions your tenant consented to when CIPP
                    was installed. A CIPP superadmin or admin role is all that
                    is required.
                  </Alert>
                  ...
                  <div>
                    ...
                    <PermissionTable
                      rows={ssoAppPermissions}
                      typeLabel="Delegated · Microsoft Graph"
                    />
                    <Typography
                      variant="caption"
                    ...
                    >
                      No application (app-only) permissions are requested, so
                      the app can never act without a signed-in user. None of
                      these scopes grant access to mail, files, Teams or
                      directory data — they are the standard OpenID Connect
                      sign-in scopes, classed by Microsoft as low impact. Who
                      can actually reach CIPP is still controlled by the CIPP
                      Users list.
                    </Typography>
                  </div>
                  ...
                  <div>
                    ...
                    <PermissionTable
                      rows={samPermissionsUsed}
                      typeLabel="Application · on CIPP-SAM"
                    />
                    <Typography
                      variant="caption"
                      color="text.secondary"
                      sx={{ display: 'block', mt: 1 }}
                    >
                      These are part of the standard CIPP-SAM permission set and
                      were consented when CIPP was installed — nothing new is
                      requested during setup. If setup fails on one of these
                      steps, your CIPP-SAM consent predates that permission and
                      needs re-consenting from the SAM App Permissions page.
                    </Typography>
                  </div>
                  ...
                  <div>
                    ...
                    <Typography variant="body2" sx={{ mb: 1 }}>
                      The most common blocker is a tenant app management policy
                      that forbids adding client secrets.{' '}
                      <strong>Repair</strong> retries it, and CIPP will try to
                      exempt itself from the policy. If it still fails, an Entra
                      administrator can create the CIPP-SSO app registration by
                      hand and you can store its credentials under{' '}
                      <strong>Manual configuration</strong> below — the
                      documentation has the exact settings to use.
                    </Typography>
                    <Typography variant="body2">
                      If sign-in is broken badly enough that you cannot reach
                      this page at all, use <strong>Reset SSO</strong> in the{' '}
                      <Link
                      ...
                      . That returns the instance to its setup wizard, which is
                  ...
                  <Link
                    ...
                    variant="body2"
                  >
                    Full SSO documentation, including permission justifications
                  ...
                  <Grid size={{ xs: 12, md: 8 }}>
                    ...
                    >
                      {signInHosts.map((host) => (
                        <Chip
                          ...
                          size="small"
                          color={
                            missingSignInHosts.includes(host)
                              ? 'warning'
                              : 'default'
                          }
                        />
                    ...
                    {missingSignInHosts.length > 0 && (
                      ...
                      >
                        ...
                        {missingSignInHosts.length === 1 ? 'is' : 'are'} bound
                        to this instance but not registered on the app. Click{' '}
                    ...
                    {domainsUnverified && (
                      <Typography
                        ...
                        sx={{ display: 'block', mt: 0.5 }}
                      >
                        This list may be incomplete — the custom domains bound
                        to this instance could not be read, so a domain that
                        cannot sign in would not show up here.
                  ...
                  >
                    ...
                    {canRepair && (
                      <Typography
                        ...
                        sx={{ display: 'block', mt: 1 }}
                      >
                        The app registration ({data.appId}) was created
                        successfully but the client secret could not be
                        generated. Click <strong>Repair</strong> to retry the
                        secret on the existing app, or <strong>Recreate</strong>{' '}
                        to start over with a fresh app registration.
                      </Typography>
                    )}
                    {isOrphanedError && (
                      <Typography
                        ...
                        sx={{ display: 'block', mt: 1 }}
                      >
                        A previous attempt to set up SSO did not save an App ID,
                        so there's nothing to repair. An orphaned{' '}
                        <strong>CIPP-SSO</strong> app registration may exist in
                        your Entra tenant — you can delete it manually. Click{' '}
                        <strong>Create SSO App</strong> to provision a fresh app
                        registration.
                      </Typography>
                    )}
                  </Alert>
            ...
            >
              ...
              <AccordionDetails sx={{ px: 0 }}>
                <Stack spacing={2}>
                  <Alert severity="info">
                    Enter an existing Application (client) ID and client secret
                    to store them directly in Key Vault — for example to rotate
                    the secret by hand, or to point SSO at an app registration
                    an Entra administrator created for you because a tenant
                    policy blocks CIPP from creating one. The instance must be
                    restarted for the change to take effect.{' '}
                    <Link
                      href={`${SSO_DOCS_URL}#creating-the-app-registration-manually`}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
