...
export const CippTenantSelector = React.forwardRef((props, ref) => {
  ...
  const baseTenantOptions = useMemo(() => {
    if (
      ...
      !Array.isArray(tenantList.data) ||
      tenantList.data.length === 0
    ) {
      return []
    }
    return tenantList.data.map(
      ({
        customerId,
        displayName,
        defaultDomainName,
  ...
  const groupedTenantOptions = useMemo(() => {
    ...
    const allTenantsOption = baseTenantOptions.find(
      (option) => option.value === 'AllTenants'
    )
    const selectableOptions = baseTenantOptions.filter(
      (option) => option.value !== 'AllTenants'
    )
    ...
    const favoriteValues = new Set(
      favorites
        .map((item) => item.value)
        .filter((value) => value !== 'AllTenants')
    )
    const recentValues = recent
      .map((item) => item.value)
      .filter((value) => value !== 'AllTenants' && !favoriteValues.has(value))
    ...
    const allOptions = selectableOptions
      .filter(
        (option) =>
          !favoriteValues.has(option.value) && !recentSet.has(option.value)
      )
      .slice()
  ...
  useEffect(() => {
    if (
      tenant &&
      currentTenant?.value &&
      currentTenant?.value !== 'AllTenants'
    ) {
        ...
        <CippAutoComplete
          ...
          renderOption={(props, option, { selected }) => {
            ...
            return (
              <Box
                component="li"
                key={key ?? `${option.group}-${option.value}`}
                {...optionProps}
              >
                <Box
                  sx={{
                ...
                >
                  <Typography
                    ...
                    sx={{
                      flex: 1,
                      minWidth: 0,
                      fontWeight: isAllTenants ? 600 : 400,
                    }}
                  >
                    {option.label}
                  </Typography>
                  ...
                  {!isAllTenants && (
                    <Tooltip
                      title={favourited ? 'Remove favorite' : 'Add favorite'}
                    >
                      <IconButton
                        size="small"
                        edge="end"
                        aria-label={
                          favourited ? 'Remove favorite' : 'Add favorite'
                        }
                        onClick={(event) => handleToggleFavorite(event, option)}
                        onMouseDown={(event) => event.preventDefault()}
                        sx={{
                          color: favourited ? 'warning.main' : 'action.active',
                          flexShrink: 0,
                        }}
                      >
                        {favourited ? (
