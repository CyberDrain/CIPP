...
export const CippMobileCardList = (props) => {
  ...
  const hasActiveFilter =
    Boolean(globalFilter) || (columnFilters?.length ?? 0) > 0
  ...
  const visibleDetailColumns = (row) => {
    const shown = [
      ...
      slots.secondary && textValue(row, slots.secondary),
    ]
      .filter(Boolean)
      .map((v) => v.toLowerCase())
    return slots.details.filter((col) => {
      ...
      if (typeof raw === 'string' && shown.includes(raw.toLowerCase()))
        return false
      return true
    })
  }
    ...
    <Box data-testid="cipp-mobile-card-list">
      ...
      <Stack
        ...
        sx={{
          px: fixedChrome ? 1 : 0,
          pt: 0.75,
          pb: fixedChrome ? (selectMode ? 12 : 10) : 1,
        }}
      >
        {showSkeletons ? (
              ...
              return (
                <Card
                  ...
                  sx={{
                    ...
                    position: 'relative',
                    cursor:
                      selectMode || openOffCanvasOnTap ? 'pointer' : 'default',
                ...
                >
                  {selectMode && (
                    <Checkbox
                      ...
                      inputProps={{
                        'aria-label': `Select ${textValue(row, slots.primary) ?? row.id}`,
                      }}
                    />
                  )}
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    <Typography
                      ...
                      sx={{
                        fontWeight: 600,
                        pr: actions?.length || hasOffCanvas ? 4.5 : 0,
                      }}
                    >
                      {textValue(row, slots.primary) ?? '—'}
                    </Typography>
                    ...
                    {slots.chips.length > 0 && (
                      ...
                      >
                        {slots.chips.map((col) => {
                          ...
                          const isMuteAlone = MUTE_ALONE.has(
      ...
      <CippBottomSheet
        ...
        title={
          actionSheetRow
            ? (textValue(actionSheetRow, slots.primary) ?? 'Row actions')
            : ''
        }
      >
        {actionSheetRow &&
          rowActionItems(actionSheetRow).map((action, index) => {
            const disabled =
              isActionDisabled?.(actionSheetRow.original, action) ?? false
            return (
              <ListItemButton
