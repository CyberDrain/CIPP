...
const Page = () => {
  ...
  const backupTenantFilter = useWatch({
    control: tenantFilterForm.control,
    name: 'tenantFilter',
  })
  ...
  const filteredBackupData = Array.isArray(backupList.data)
    ? backupList.data.filter(Boolean)
    : []
  ...
  const tenantFilteredBackupData =
    ...
    selectedTenantValue !== 'AllTenants'
      ? filteredBackupData.filter(
          (backup) => backup.TenantFilter === selectedTenantValue
        )
      : filteredBackupData
  ...
  const currentConfig = Array.isArray(existingBackupConfig.data)
    ? existingBackupConfig.data.find(
        (tenant) =>
          tenant.Tenant.value === settings.currentTenant ||
          tenant.Tenant.value === 'AllTenants'
      )
    : null
  const hasExistingConfig =
    currentConfig && currentConfig.Parameters?.ScheduledBackupValues
  ...
  const configPropertyItems = hasExistingConfig
    ? [
        ...
        {
          label: 'Recurrence',
          value: currentConfig.Recurrence?.value || 'Daily',
        },
  ...
  const getEnabledComponents = () => {
    ...
    if (values.CippScriptedAlerts)
      enabledComponents.push('CIPP Scripted Alerts')
            ...
            <Stack spacing={3}>
              ...
              {existingBackupConfig.isFetching ? (
                ...
                <Alert severity="info">
                  ...
                  configuration that will apply to all tenants. A tenant
                  specific backup can exist alongside a global backup, and will
                  run according to its own schedule.
                </Alert>
              )}
            </Stack>
          </Grid>
                                ...
                                >
                                  ...
                                  >
                                    <Button
                                      ...
                                      onClick={() =>
                                        handleOpenBackupPreview(backup)
                                      }
                                    >
                                      Preview
                                    </Button>
