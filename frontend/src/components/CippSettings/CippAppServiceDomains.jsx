...
const DomainWizard = ({ open, onClose, siteInfo, initialDomain }) => {
  ...
  const plan = useMemo(
    () => computeRecordPlan(hostname, siteInfo),
    [hostname, siteInfo]
  )
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>
        {managing
          ? `Manage domain — ${initialDomain?.Hostname}`
          : 'Add custom domain'}
      </DialogTitle>
      <DialogContent>
            ...
            {hostnameValid && (
              <>
                <Alert severity="info">
                  ...
                  <strong>Check DNS</strong>. The{' '}
                  <strong>{plan.recommendedType}</strong> alias record is
                  recommended for this domain type; Azure also accepts the other
                  alias type.
                </Alert>
                <Table size="small">
                  <TableHead>
                    ...
                    {plan.records.map((r) => (
                      <TableRow key={r.purpose}>
                        ...
                        >
                          {r.value}
                          {r.value ? (
                            <CippCopyToClipBoard text={r.value} />
                          ) : null}
                        </TableCell>
                ...
                {isWildcard && (
                  <Alert severity="warning">
                    A wildcard alias can't be resolved directly, so it is
                    validated by Azure when the binding is created. Note: App
                    Service Managed Certificates do not support wildcard domains
                    — you will need to upload your own certificate for HTTPS.
                  </Alert>
                )}
                {legacyAsuid && (
                  <Alert severity="warning">
                    A leftover TXT record was found at{' '}
                    <code>{dnsResult?.LegacyAsuidHost}</code>. CIPP no longer
                    uses domain-verification TXT records —{' '}
                    <strong>remove it</strong>. A stale record blocks Azure's
                    validation even when the alias record is correct.
                  </Alert>
                )}
                {dnsResult && !canProceed && (
                  <Alert severity="warning">
                    The alias record hasn't propagated yet — DNS changes can
                    take a few minutes. If the record is proxied (e.g.
                    Cloudflare orange-cloud), Azure can't see it: set it to
                    DNS-only until the domain is bound and the certificate is
                    issued. {dnsResult.AliasDetail}
                  </Alert>
                )}
                {dnsCheck.isError && (
        ...
        {activeStep === 1 && (
          <Stack spacing={2}>
            <Alert severity="info">
              Create the hostname binding on the App Service for{' '}
              <strong>{hostname}</strong>. Azure re-validates the DNS records
              during this step.
            </Alert>
            {bindingDone ? (
              <Alert
          ...
          <Stack spacing={2}>
            {certDone ? (
              <Alert severity="success" icon={<Lock fontSize="inherit" />}>
                <strong>{hostname}</strong> is fully configured and secured with
                a managed certificate.
              </Alert>
            ) : isWildcard ? (
              <Alert severity="warning">
                App Service Managed Certificates don't support wildcard domains.
                Upload your own certificate and binding from the Azure Portal to
                secure <strong>{hostname}</strong>.
              </Alert>
            ) : (
              <>
                <Alert severity="info">
                  Provision a free App Service Managed Certificate for{' '}
                  <strong>{hostname}</strong> and enable the SNI SSL binding.
                  This can take a minute or two.
                </Alert>
                <Alert severity="warning">
                  If the domain's alias is proxied through a CDN (e.g.
                  Cloudflare orange-cloud), temporarily set it to DNS-only while
                  the certificate is issued, then re-enable the proxy
                  afterwards. Certificate issuance validates the domain
                  directly.
                </Alert>
              </>
            )}
        ...
        <Stack direction="row" spacing={1}>
          ...
          {activeStep === 1 && (
            <Button
              ...
              startIcon={
                bindingAction.isPending ? <CircularProgress size={16} /> : null
              }
            >
              {bindingDone
                ? 'Next'
                : bindingAction.isPending
                  ? 'Creating...'
                  : 'Create binding'}
            </Button>
          )}
          ...
          {activeStep === 2 && !certDone && !isWildcard && (
            <Button
              ...
              startIcon={
                certAction.isPending ? <CircularProgress size={16} /> : <Lock />
              }
            >
              {certAction.isPending
                ? 'Provisioning...'
                : 'Provision certificate & enable HTTPS'}
            </Button>
          )}
...
export const CippAppServiceDomains = () => {
  ...
  return (
    <Grid container spacing={3}>
      <Grid size={{ xs: 12 }}>
        <Alert severity="info">
          Map custom domains to the App Service that hosts this CIPP instance.
          Each domain needs a DNS alias record, a hostname binding, and
          (optionally) a free managed TLS certificate — the wizard walks through
          all three and can be reopened at any time to finish or fix a domain.
          The default <code>*.azurewebsites.net</code> hostname always remains
          available.
        </Alert>
      </Grid>
        ...
        >
          <CardContent>
            {domainsQuery.isLoading ? (
              ...
              <Alert severity="error">
                Could not load App Service details. Ensure the managed identity
                has access to the resource group.
              </Alert>
            ) : (
              <Stack spacing={1.5}>
                <InfoRow
                ...
                <Typography variant="caption" color="text.secondary">
                  ...
                  longer uses domain-verification TXT records — remove any
                  leftover <code>asuid.&lt;domain&gt;</code> record.
                </Typography>
              </Stack>
            )}
          </CardContent>
