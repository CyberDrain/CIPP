...
const CippBrandingSettings = () => {
  ...
  const branding =
    brandingQuery.data && !Array.isArray(brandingQuery.data)
      ? brandingQuery.data
      : {}

  const [logoImageId, setLogoImageId] = useState(branding.logoImageId || null)
  const [logoImageIds, setLogoImageIds] = useState(() =>
    normalizeLogoImageIds(branding)
  )
  const [logoUploads, setLogoUploads] = useState(() =>
    normalizeLogoUploads(branding)
  )
  const [coverStock, setCoverStock] = useState(
    branding.coverStock || DEFAULT_COVER_STOCK
  )
  const [coverImageId, setCoverImageId] = useState(
    branding.coverImageId || null
  )
  const [coverImageIds, setCoverImageIds] = useState(() =>
    normalizeCoverImageIds(branding)
  )
  const [coverUploads, setCoverUploads] = useState(() =>
    normalizeCoverUploads(branding)
  )
  ...
  useEffect(() => {
    ...
    const coversHydrated = !(
      nextCoverIds.length > 0 && nextCoverUploads.length === 0
    )
    const logosHydrated = !(
      nextLogoIds.length > 0 &&
      nextLogoUploads.length === 0 &&
      !next.logo
    )
    ...
    if (next.colour) {
      formControl.reset({
        ...
        previewReportType:
          formControl.getValues('previewReportType') || reportTypeOptions[0],
      })
    }
  ...
  const previewReportType =
    previewReportTypeValue?.value ||
    previewReportTypeValue ||
  ...
  const noneOption = useMemo(() => {
    const option = COVER_STOCK_OPTIONS.find(
      (item) => item.path === COVER_STOCK_NONE
    )
      ...
      logoImageIds
        ...
        .filter(
          (option) =>
            ...
            option.src.startsWith('data:image/')
        ),
  ...
  const stockOptions = useMemo(
    () =>
      COVER_STOCK_OPTIONS.filter(
        (option) => option.path !== COVER_STOCK_NONE
      ).map((option) => ({
        key: option.key,
        src: option.path,
        label: option.label,
  ...
  const buildLocalBranding = (overrides = {}) => {
    ...
    const nextCoverId =
      overrides.coverImageId !== undefined
    ...
    const nextLogoId =
      overrides.logoImageId !== undefined ? overrides.logoImageId : logoImageId
  ...
  const handleLogoUpload = (event) => {
    ...
    readImageFile(file, async (base64String) => {
      ...
      try {
        ...
        const nextIds = [
          id,
          ...logoImageIds.filter((existing) => existing !== id),
        ]
        const nextUploads = [
          base64String,
  ...
  const handleLogoDelete = async (id) => {
    ...
    try {
      ...
      const nextUploads = logoUploads.filter(
        (_, index) => logoImageIds[index] !== id
      )
      setLogoImageIds(nextIds)
      setLogoUploads(nextUploads)
  ...
  const handleCoverUpload = (event) => {
    ...
    readImageFile(file, async (base64String) => {
      ...
      try {
        ...
        const nextIds = [
          id,
          ...coverImageIds.filter((existing) => existing !== id),
        ]
        const nextUploads = [
          base64String,
  ...
  const handleCoverDelete = async (id) => {
    ...
    try {
      ...
      const nextUploads = coverUploads.filter(
        (_, index) => coverImageIds[index] !== id
      )
      setCoverImageIds(nextIds)
      setCoverUploads(nextUploads)
  ...
  const handleSelectScope = (presetId) => {
    ...
    setCoverStock(
      source?.coverStock || (presetId ? COVER_STOCK_NONE : DEFAULT_COVER_STOCK)
    )
    pinLogoSelection(source?.logoImageId || null)
    pinCoverSelection(
      source?.coverImageId || null,
      source?.coverStock || COVER_STOCK_NONE
    )
  }
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      <Box>
        ...
        <Typography variant="body2" color="text.secondary">
          Customize your organization&apos;s branding for reports and documents.
          The default applies to every report; presets can be assigned to
          individual report templates instead.
        </Typography>
      </Box>
          ...
          {REPORT_COVER_PRESETS.map((report) => (
            <Grid key={report.id} size={{ xs: 12, sm: 6, md: 4 }}>
              <CippAutoComplete
                ...
                value={
                  presetOptions.find(
                    (option) => option.value === reportDefaults[report.id]
                  ) ?? presetOptions[0]
                }
                onChange={(option) =>
                  handleReportDefaultChange(report.id, option?.value ?? '')
                }
              />
            </Grid>
          ))}
      ...
      >
        ...
        <DialogContent>
          ...
          <Box
            onKeyDown={(event) => event.key === 'Enter' && handleConfirmName()}
          >
            <CippFormComponent
              type="textField"
              name="presetName"
            ...
            >
              <Box>
                ...
                >
                  ...
                  <Button
                    size="small"
                    onClick={() => setLivePreview((open) => !open)}
                  >
