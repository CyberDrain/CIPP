...
export const BackupValidator = {
    ...
    try {
      ...
      try {
        ...
        result.warnings.push(
          `Initial JSON parsing failed: ${parseError.message}`
        )
  ...
  _attemptAdvancedRepair(jsonString, parseError) {
    ...
    try {
      ...
      if (positionMatch) {
        ...
        result.errors.push(
          `Attempting to repair corruption at position ${errorPosition}`
        )

        // Strategy 1: Try to find and isolate the corrupted entry
        const isolatedResult = this._isolateCorruptedEntry(
          jsonString,
          errorPosition
        )
  ...
  _isolateCorruptedEntry(jsonString, errorPosition) {
    ...
    try {
      ...
      if (lastObjectStart !== -1 && nextObjectStart !== -1) {
        ...
        const afterCorrupted = jsonString.substring(
          errorPosition + nextObjectStart
        )
        ...
        try {
          ...
          result.repairedJson = repaired
          result.errors.push(
            'Successfully isolated and removed corrupted entry'
          )
          return result
        } catch (stillError) {
      ...
      if (lastCompleteObject !== -1) {
        ...
        try {
          ...
          if (Array.isArray(parsed) && parsed.length > 0) {
            ...
            result.errors.push(
              `Extracted ${parsed.length} valid entries before corruption`
            )
            return result
          }
        } catch (stillError) {
          result.errors.push(
            `Valid portion extraction failed: ${stillError.message}`
          )
        }
      }
    } catch (error) {
  ...
  _validateImportableData(data) {
    ...
    if (!Array.isArray(data)) {
      if (data && typeof data === 'object') {
        ...
        data = [data]
        result.warnings.push(
          'Single object detected, converted to array format'
        )
      } else {
    ...
    data.forEach((row, index) => {
      if (this._isValidImportRow(row)) {
        ...
        result.skippedRows++
        result.warnings.push(
          `Row ${index + 1} skipped: ${this._getRowSkipReason(row)}`
        )
      }
    })
  ...
  _isValidImportRow(row) {
    ...
    const hasPartitionKey =
      row.PartitionKey && typeof row.PartitionKey === 'string'
