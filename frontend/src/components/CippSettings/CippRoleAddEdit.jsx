...
export const CippRoleAddEdit = ({ selectedRole }) => {
  ...
  const blockedTenants = useWatch({
    control: formControl.control,
    name: 'blockedTenants',
  })
  const blockedEndpoints = useWatch({
    control: formControl.control,
    name: 'BlockedEndpoints',
  })
  const setDefaults = useWatch({
    control: formControl.control,
    name: 'Defaults',
  })
  ...
  const selectedEntraGroup = useWatch({
    control: formControl.control,
    name: 'EntraGroup',
  })
  const ipRanges = useWatch({ control: formControl.control, name: 'IPRange' })
  const includeRules = useWatch({
    control: formControl.control,
    name: 'PermissionRulesInclude',
  })
  const excludeRules = useWatch({
    control: formControl.control,
    name: 'PermissionRulesExclude',
  })
  const baseRoleTemplate = useWatch({
    control: formControl.control,
  ...
  useEffect(() => {
    ...
    const toOptions = (list) =>
      (list || []).map((pattern) => ({ label: pattern, value: pattern }))
    formControl.setValue(
      'PermissionRulesInclude',
      toOptions(cippRoles[roleName].include)
    )
    formControl.setValue(
      'PermissionRulesExclude',
      toOptions(cippRoles[roleName].exclude)
    )
  ...
  const permissionUniverse = useMemo(
    () => flattenPermissionTree(apiPermissions),
    [apiPermissions]
  )
  const ruleSuggestions = useMemo(
    () => buildRuleSuggestions(apiPermissions),
    [apiPermissions]
  )
  ...
  const coreCovered = ruleExpansion.matched.some((p) =>
    p.startsWith('CIPP.Core.')
  )
  ...
  const handleModeChange = (_event, newMode) => {
    ...
    if (newMode === 'advanced') {
      ...
      if (currentRules.Include.length > 0) {
        formControl.setValue(
          'Permissions',
          rulesToFlatMap(currentRules, apiPermissions)
        )
      }
      ...
      const diverged =
        ...
        Object.keys(rulesGrid).some(
          (key) => (selectedPermissions?.[key] ?? null) !== rulesGrid[key]
        )
  ...
  const getBaseRolePermissions = (role) => {
    ...
    Object.keys(apiPermissions).forEach((cat) => {
      Object.keys(apiPermissions[cat]).forEach((obj) => {
        ...
        if (
          (includeRead || includeReadWrite) &&
          !(excludeRead || excludeReadWrite)
        ) {
  ...
  useEffect(() => {
    if (
      ...
      Object.entries(getBaseRolePermissions(selectedRole)).forEach(
        ([cat, objects]) => {
      ...
      if (currentPermissions) {
        ...
        const hasWildcards = storedRules?.Include?.some((pattern) =>
          pattern.includes('*')
        )
  ...
  const handleSubmit = () => {
        ...
        ?.map((tenant) => {
          if (tenant.type === 'Group') {
            ...
            const tenantInfo = tenants.find(
              (t) => t.defaultDomainName === tenant.value
            )
    ...
    const snapshotPermissions =
      permissionMode === 'simple'
        ? rulesToFlatMap(activeRules, apiPermissions)
        : selectedPermissions
  ...
  const ApiPermissionRow = ({ obj, cat, readOnly }) => {
    ...
    const [descriptionOffcanvasVisible, setDescriptionOffcanvasVisible] =
      useState(false)
        ...
        >
          <Stack spacing={2}>
            <Typography variant="body1" sx={{ mx: 3 }}>
              Listed below are the available API endpoints based on permission
              level. ReadWrite level includes endpoints under Read.
            </Typography>
            {Object.keys(apiPermissions[cat][obj]).map((type, typeIndex) => {
              ...
              for (var api in apiPermissions[cat][obj][type]) {
                ...
                items.push({
                  ...
                  description: getFunctionDescriptionText(
      ...
      <Grid container spacing={3}>
        <Grid size={{ xs: 12, xl: 9 }}>
          <Stack spacing={1} sx={{ mb: 3 }}>
            ...
            {selectedRole &&
              isBaseRole &&
              ['admin', 'superadmin'].includes(selectedRole) && (
                <Alert color="warning" icon={<WarningOutlined />}>
                  This is a highly privileged role and overrides any custom role
                  restrictions.
                </Alert>
              )}
            {cippApiRoleSelected && (
              <Alert color="info">
                This is the default role for all API clients in the CIPP-API
                integration. If you would like different permissions for
                specific applications, create a role per application and select
                it from the CIPP-API integrations page.
              </Alert>
            )}
            <CippFormComponent
              type="autoComplete"
              ...
              <Stack spacing={1} sx={{ my: 3 }}>
                ...
                {allTenantSelected && blockedTenants?.length == 0 && (
                  <Alert color="warning">
                    ...
                    unless blocked tenants are specified.
                  </Alert>
                )}
              </Stack>
              {allTenantSelected && (
                          ...
                          Object.keys(apiPermissions)
                            ...
                            .forEach((cat) => {
                              Object.keys(apiPermissions[cat])
                                .sort()
                                .forEach((obj) => {
                                  Object.keys(apiPermissions[cat][obj]).forEach(
                                    (type) => {
                                      Object.keys(
                                        apiPermissions[cat][obj][type]
                                      ).forEach((apiKey) => {
                                        const apiFunction =
                                          apiPermissions[cat][obj][type][apiKey]
                                        const descriptionText =
                                          getFunctionDescriptionText(
                                            apiFunction.Description
                                          )
                                        allEndpoints.push({
          ...
          {apiPermissionSuccess && (
            <>
              ...
              {isBaseRole &&
                selectedRole &&
                cippRoles[selectedRole]?.include && (
                  <>
                    ...
                    <Box sx={{ mb: 3 }}>
                      ...
                      <Typography variant="body2" sx={{ mb: 2 }}>
                        These patterns define which permissions are included for
                        this base role:
                      </Typography>
                      <Box
                        sx={{
                      ...
                      {cippRoles[selectedRole]?.exclude &&
                        cippRoles[selectedRole].exclude.length > 0 && (
                          <>
                            ...
                            <Typography variant="body2" sx={{ mb: 2 }}>
                              These patterns define which permissions are
                              explicitly excluded from this base role:
                            </Typography>
                            <Box
                              sx={{
                                fontFamily: 'monospace',
                            ...
                            >
                              {cippRoles[selectedRole].exclude.map(
                                (pattern, idx) => (
                                  <Box
                                    key={idx}
                                    sx={{
              ...
              {!isBaseRole && permissionMode === 'simple' && (
                <Stack spacing={2} sx={{ mb: 3 }}>
                  <Alert color="info">
                    Simple mode works like CIPP's built-in roles: pick what to
                    include, then carve out exclusions. Wildcards (*) match
                    anything, so rules automatically cover new features added in
                    future CIPP releases.
                  </Alert>
                  {gridDiverged && (
                    <Alert color="warning">
                      Changes made in Advanced mode are not reflected in these
                      patterns. Saving in Simple mode will replace the role's
                      permissions with the patterns below.
                    </Alert>
                  )}
                  <CippFormComponent
                    type="autoComplete"
                  ...
                  {[...currentRules.Include, ...currentRules.Exclude]
                    ...
                    .map((pattern) => (
                      <Alert color="error" key={`invalid-${pattern}`}>
                        "{pattern}" is not a valid pattern. Use up to three
                        dot-separated segments of letters, numbers and *, e.g.
                        Identity.User.Read or Exchange.*.
                      </Alert>
                    ))}
                  <Box>
                    ...
                    >
                      {currentRules.Include.map((pattern) => (
                        <Chip
                          ...
                          label={`${pattern} — ${ruleExpansion.includeCounts[pattern] ?? 0} match${
                            (ruleExpansion.includeCounts[pattern] ?? 0) === 1
                              ? ''
                              : 'es'
                          }`}
                          color={
                          ...
                          icon={
                            (ruleExpansion.includeCounts[pattern] ?? 0) ===
                            0 ? (
                              <WarningOutlined />
                            ) : undefined
                          }
                      ...
                      {currentRules.Exclude.map((pattern) => (
                        <Chip
                          ...
                          icon={
                            (ruleExpansion.excludeCounts[pattern] ?? 0) ===
                            0 ? (
                              <WarningOutlined />
                            ) : undefined
                          }
                    ...
                    {currentRules.Include.length === 0 ? (
                      <Alert color="warning">
                        Add at least one include pattern — a role with no
                        inclusions grants no access and cannot be saved.
                      </Alert>
                    ) : (
                      <Stack direction="row" spacing={2} alignItems="center">
                        ...
                        <Button
                          size="small"
                          onClick={() => setRulePreviewVisible(true)}
                        >
                    ...
                    {currentRules.Include.length > 0 && !coreCovered && (
                      <Alert color="info" sx={{ mt: 1 }}>
                        CIPP.Core.Read is required to sign in and will be added
                  ...
                  >
                    <Stack spacing={1} sx={{ mx: 3 }}>
                      <Typography variant="body2" sx={{ mb: 1 }}>
                        Permissions granted by the current patterns — expand one
                        to see the API endpoints it serves. Struck-through
                        entries were matched by an include pattern but removed
                        by an exclusion.
                      </Typography>
                      {ruleExpansion.matched.map((permission) => {
                        const [permCat, permObj, permType] =
                          permission.split('.')
                        ...
                        const sections = [
                          {
                            type: permType,
                            endpoints:
                              apiPermissions?.[permCat]?.[permObj]?.[permType],
                          },
                        ]
                        if (
                          permType === 'ReadWrite' &&
                          apiPermissions?.[permCat]?.[permObj]?.Read &&
                          !ruleExpansion.matched.includes(
                            `${permCat}.${permObj}.Read`
                          )
                        ) {
                          sections.push({
                        ...
                        const endpointCount = sections.reduce(
                          (total, section) =>
                            total + Object.keys(section.endpoints || {}).length,
                          0
                        )
                        return (
                          <Accordion
                              ...
                              <Stack spacing={1}>
                                {sections.map((section) => (
                                  <React.Fragment key={section.type}>
                                    ...
                                    {Object.keys(section.endpoints || {}).map(
                                      (apiKey) => {
                                        const apiFunction =
                                          section.endpoints[apiKey]
                                        const description =
                                          getFunctionDescriptionText(
                                            apiFunction.Description
                                          )
                      ...
                      {Object.entries(ruleExpansion.excludedBy).map(
                        ([permission, pattern]) => (
                          <Typography
                            key={permission}
                            variant="body2"
                          ...
                          >
                            ...
                            <AccordionDetails>
                              {Object.keys(apiPermissions[cat])
                                .sort()
                                .map((obj, index) => {
                                  const readOnly = baseRolePermissions?.[cat]
                                    ? true
                                    : false
                                  return (
                                    <Grid
