...
const matchesWildcardPattern = (queryKey, pattern) =>
  wildcardToRegExp(pattern).test(queryKey)
      ...
      if (bulkRequest && Array.isArray(data)) {
        ...
        if (relatedQueryKeys) {
          ...
          setTimeout(() => {
            ...
            const wildcardPatterns = clearKeys.filter((key) =>
              key.includes('*')
            )
...
export function ApiGetCallWithPagination({
  ...
  const queryInfo = useInfiniteQuery({
    ...
    getNextPageParam: (lastPage) => {
      ...
      return lastPage?.Metadata?.nextLink
        ? { nextLink: lastPage.Metadata.nextLink }
        : undefined
    },
    staleTime: 300000,
    refetchOnWindowFocus: false,
