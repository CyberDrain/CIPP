    ...
    try {
      ...
      const formattedTimezones = tzData
        ...
        .map((tz) => {
          ...
          const current = tz?.currentTimeFormat
            ? String(tz.currentTimeFormat)
            : undefined
        ...
        .filter(
          (item, idx, arr) =>
            arr.findIndex((t) => t.value === item.value) === idx
        )
      ...
      setTimezones((prev) =>
        prev?.length ? prev : [{ label: 'UTC', value: 'UTC' }]
      )
    } finally {
      setLoading(false)
    }
