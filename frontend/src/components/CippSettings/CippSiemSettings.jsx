  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1 }}>
        <Alert severity="info">
          Generate a read-only SAS token for the CIPP Logs table. This token can
          be used to query log data from external SIEM tools or scripts using
          the Azure Table Storage REST API. Note that generating a new URL does
          not invalidate previous URLs.
        </Alert>
        ...
        {generateSas.isSuccess && generateSas.data?.data?.Results && (
          <>
            <Alert severity="success">
              SAS URL generated successfully. Copy this for your records, it
                  ...
                  input: {
                    ...
                    endAdornment: (
                      <InputAdornment position="end">
                        <CippCopyToClipBoard
                          text={generateSas.data.data.Results.SASUrl}
                        />
                      </InputAdornment>
                    ),
                  },
            ...
            <Box>
              ...
              <Typography variant="body2">
                {new Date(
                  generateSas.data.data.Results.ExpiresOn
                ).toLocaleString()}
              </Typography>
            </Box>
          </>
