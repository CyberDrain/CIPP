...
describe('WizardSteps', () => {
  it('keeps the full stepper on desktop', () => {
    renderWithProviders(
      <WizardSteps orientation="horizontal" activeStep={2} steps={steps} />
    )
  ...
  it('leaves the vertical status list alone on a phone', () => {
    ...
    const onboarding = [
      ...
      {
        title: 'roles',
        description: 'Role assignment failed: insufficient privileges',
        error: true,
      },
    ]
    renderWithProviders(
      <WizardSteps orientation="vertical" activeStep={1} steps={onboarding} />
    )
  ...
  it("carries the current step's error and loading states into the bar", () => {
    ...
    renderWithProviders(
      <WizardSteps orientation="horizontal" activeStep={0} steps={running} />
    )
  ...
  it('renders nothing broken for an empty step list', () => {
    layoutState.isMobile = true
    renderWithProviders(
      <WizardSteps orientation="horizontal" activeStep={0} steps={[]} />
    )
