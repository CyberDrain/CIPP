    ...
    <>
      ...
      >
        <Grid container spacing={2} sx={{ mb: 2 }}>
          <Grid size={12}>
            <Alert severity="info">
              This page is used to offboard a tenant. Please select the tenant
              and the actions to be performed. Please note that once an
              offboarding has been executed, it cannot be undone.
            </Alert>
          </Grid>
          <Grid size={12}>
            <CippFormComponent
          ...
          {tenantId && (
            <>
              ...
              <Grid size={12}>
                <CippInfoBar
                  isFetching={
                    gdapRelationships.isFetching || cspContracts.isFetching
                  }
                  data={[
                    {
                    ...
                    },
                    {
                      ...
                      offcanvas: {
                        ...
                        propertyItems: vendorApps.data?.pages
                          ?.reduce(
                            (sum, page) => sum.concat(page?.Results ?? []),
                            []
                          )
                          .map((app) => ({
                            label: app?.displayName,
              ...
              <Grid size={12}>
                ...
                <Typography variant="subtitle2" color="text.secondary">
                  The tenant will not be fully offboarded unless all the
                  relationships/contracts are terminated.
                </Typography>
              </Grid>
              <Grid size={{ xs: 12, md: 6 }}>
                <Stack spacing={0.5}>
                  <CippFormComponent
                    ...
                    api={{
                      ...
                      labelField: (app) => {
                        const vendor = vendorTenantList.find(
                          (v) =>
                            v?.vendorTenantId === app?.appOwnerOrganizationId
                        )
                    ...
                    disabled={
                      vendorApps?.data?.pages?.[0]?.Results?.length > 0
                        ? false
                        : true
                    }
                  />
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                <Alert severity="error" sx={{ p: 2 }}>
                  These actions will terminate all delegated access to the
                  customer tenant!
                  <Divider sx={{ my: 2 }} />
                  <Box sx={{ pl: 1 }}>
                    <Stack spacing={0.5}>
                      <CippFormComponent
                        ...
                        type="switch"
                        disabled={
                          mspApps?.data?.Results?.length > 0 ? false : true
                        }
                      />
                      <CippFormComponent
                        formControl={formControl}
                        ...
                        disabled={
                          gdapRelationships?.data?.Results?.find(
                            (relationship) =>
                              relationship?.customer?.tenantId ===
                              tenantId.value
                          )
                            ? false
                            : true
