...
const Page = () => {
  ...
  const showGdapCheck =
    !partnerCheckComplete || isPartner || Boolean(importReport?.GDAP)
  ...
  return (
    ...
    >
      <Grid container spacing={2}>
        ...
        <Grid
          size={{ xs: 12 }}
          sx={{ display: { xs: 'contents', lg: 'block' } }}
        >
          <CippPermissionReport
        ...
        <Grid size={{ lg: 6, md: 12, sm: 12, xs: 12 }}>
          {showGdapCheck ? (
            ...
            <Alert severity="info">
              GDAP checks do not apply to this environment. Your tenants are
              added directly rather than through Microsoft Partner Center
              relationships, so access is verified per tenant in the Tenants
              check below.
            </Alert>
          )}
        </Grid>
