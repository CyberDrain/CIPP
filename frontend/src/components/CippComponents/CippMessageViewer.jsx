...
export const CippMessageViewer = ({ emailSource }) => {
  ...
  const EmailButtons = (emailHeaders, emailSource) => {
    ...
    const url = URL.createObjectURL(blob)
    const forefrontHeader = emailHeaders?.match(
                ...
                <Grid size={{ xs: 12, md: 9 }}>
                  <Box display="flex" alignItems="center">
                    ...
                    >
                      ...
                      {(() => {
                        const authResults =
                          emlContent?.headers?.['Authentication-Results'] || ''
                        const dmarcPass = authResults
                          ? authResults.includes('dmarc=pass')
                          : false
                        const dkimPass = authResults
                          ? authResults.includes('dkim=pass')
                          : false
                        const spfPass = authResults
                          ? authResults.includes('spf=pass')
                          : false
                        const arcPass = authResults
                          ? authResults.includes('arc=pass')
                          : false
                        const allPass =
                          dmarcPass && dkimPass && spfPass && arcPass
                        const somePass =
                          dmarcPass || dkimPass || spfPass || arcPass
        ...
        {inputMode === 'paste' && (
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            <Tooltip title={pasteCollapsed ? 'Show input' : 'Hide input'}>
              <IconButton
                size="small"
                onClick={() => setPasteCollapsed((prev) => !prev)}
              >
                <SvgIcon fontSize="small">
