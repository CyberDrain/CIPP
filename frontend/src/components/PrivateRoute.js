...
export const PrivateRoute = ({ children, routeType }) => {
  ...
  if (
    null !== apiRoles?.data?.clientPrincipal &&
    undefined !== apiRoles?.data
  ) {
  ...
  if (null === roles) {
    ...
    const userRoles =
      roles?.filter((role) => !blockedRoles.includes(role)) ?? []
    ...
    if (
      apiRoles.data?.initialSetupComplete === false &&
      router.pathname !== '/authredirect'
    ) {
