...
export const CippCoverageHeatmap = ({ rows = [], onCellClick }) => {
  ...
  const pageRows =
    rowsPerPage === -1
      ? grid
      : grid.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
  ...
  const tip = (tenant, colIdx, state, cellRows) => {
    ...
    if (cellRows.length) {
      const searchStatus = cellRows
        .map((r) => r.SearchStatus)
        .filter(Boolean)
        .slice(-1)[0]
      if (searchStatus && (state === 'InFlight' || state === 'Retrying'))
        lines.push(`search: ${searchStatus}`)
      const recs = cellRows.reduce(
        (a, r) => a + (Number(r.RecordCount) || 0),
        0
      )
      const matched = cellRows.reduce(
        (a, r) => a + (Number(r.MatchedCount) || 0),
        0
      )
      const retries = cellRows.reduce(
        (a, r) => a + (Number(r.RetryCount) || 0),
        0
      )
      const throttles = cellRows.reduce(
        (a, r) => a + (Number(r.ThrottleCount) || 0),
        0
      )
      const lastErr = cellRows
        .map((r) => r.LastError)
        .filter(Boolean)
        .slice(-1)[0]
      lines.push(`${recs} records${matched ? ` - ${matched} matched` : ''}`)
      if (retries || throttles)
        lines.push(`retries ${retries} - throttles ${throttles}`)
                  ...
                  <Box
                    ...
                    onClick={() =>
                      ...
                      onCellClick({
                        tenant: t.name,
                        startMs: columns[i],
                        endMs: t.cells[i].length
                          ? toMs(t.cells[i][0].WindowEnd)
                          : null,
                      })
                    }
                    sx={{
