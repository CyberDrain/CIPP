...
const TABLE_LABELS = {
  ...
  'templates:IntuneReusableSettingTemplate':
    'Intune Reusable Setting Templates',
...
export const CippRestoreWizard = ({
  ...
  const categories = useMemo(() => {
    ...
    return Object.entries(counts)
      ...
      .sort((a, b) =>
        (TABLE_LABELS[a[0]] || a[0]).localeCompare(TABLE_LABELS[b[0]] || b[0])
      )
  ...
  const filteredData = useMemo(
    () =>
      Array.isArray(backupData)
        ? backupData.filter(
            (item) => selectedCategories[getItemCategoryKey(item)]
          )
        : [],
  ...
  const handleConfirmRestore = () => {
    const selectedTypes = Object.keys(selectedCategories).filter(
          ...
          >
            <Stack spacing={0.5}>
              ...
              {backupFile.lastModified && (
                <Typography variant="body2">
                  <strong>Date:</strong>{' '}
                  {backupFile.lastModified.toLocaleString()}
                </Typography>
              )}
            </Stack>
          </Box>
      ...
      {validationResult &&
        (validationResult.isValid ? (
          <Alert severity="success" icon={<CheckCircle />}>
            ...
            <Typography variant="body2">
              {validationResult.validRows} valid rows across {categories.length}{' '}
              categories.
              {validationResult.repaired &&
                ' Minor issues were automatically repaired.'}
            </Typography>
        ...
        <Stack spacing={0}>
          {categories.map((cat) => {
            const items = Array.isArray(backupData)
              ? backupData.filter(
                  (item) => getItemCategoryKey(item) === cat.key
                )
              : []
              ...
              <Box key={cat.key} sx={{ width: '100%', minWidth: 0 }}>
                ...
                >
                  ...
                  <Tooltip title={isExpanded ? 'Hide items' : 'Preview items'}>
                    <IconButton
                      size="small"
                      onClick={() => handleToggleExpand(cat.key)}
                    >
                      {isExpanded ? (
    ...
    <Stack spacing={2}>
      {!restoreAction.isSuccess && (
        <Alert severity="warning" icon={<Warning />}>
          ...
          This will overwrite your current CIPP configuration for the selected
      ...
      <Box>
        <Typography variant="subtitle2" gutterBottom>
          ...
          {filteredData.length} items) selected for restore:
        </Typography>
        <Stack
          useFlexGap
          direction="row"
  ...
  return (
    ...
    >
      ...
      <DialogContent
        dividers={true}
        sx={{ overflowY: 'auto', flex: 1, px: 3, pt: 2 }}
      >
        {isLoading ? (
          <Box
            sx={{
