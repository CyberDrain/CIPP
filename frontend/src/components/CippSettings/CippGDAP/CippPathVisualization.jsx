...
export const CippPathVisualization = ({
  ...
  if (!membershipPath || membershipPath.length === 0) {
    ...
    const nodes = [
        ...
        chips: [
          {
            ...
            sx: {
              backgroundColor: isMember ? colors.success : colors.error,
              color: 'white',
            },
            size: 'small',
          },
      ...
      },
      {
        ...
        subLabel: 'GDAP Role',
        icon: (
          <Security sx={{ color: isMember ? colors.success : colors.grey }} />
        ),
        ...
        chips: [
          {
            ...
            sx: {
              backgroundColor: isMember ? colors.success : colors.grey,
              color: 'white',
            },
            size: 'small',
          },
  ...
  sortedPath.forEach((step, index) => {
    ...
    if (isNotMember) {
      ...
      if (isDirect) {
        ...
        subLabel =
          sortedPath.length === 1
            ? 'GDAP Mapped Group (User Nested)'
            : 'GDAP Mapped Group (Nested)'
      } else {
    ...
    nodes.push({
      ...
      icon: isNotMember ? (
        ...
        <Group
          sx={{ color: groupColor, fontSize: isLastGroup ? '2rem' : '1.5rem' }}
        />
      ),
  ...
  const hasAccess = !sortedPath.some(
    (step) => step.membershipType === 'not_member'
  )
  ...
  return (
    <Box sx={{ my: 2 }} {...other}>
      {relationshipName && (
        <Paper
          elevation={0}
          sx={{ p: 1, mb: 2, bgcolor: 'background.default' }}
        >
          <Stack
            useFlexGap
            direction="row"
