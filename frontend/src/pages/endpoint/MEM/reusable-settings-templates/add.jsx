...
const Page = () => {
  ...
  const baseSettingDefinitionId =
    'vendor_msft_firewall_mdmstore_dynamickeywords_addresses_{id}'
  ...
  const buildInitialGroupEntry = () =>
    buildGroupEntryFromBase(baseSettingDefinitionId, {
      idValue: generateGuid(),
    })
  ...
  const customDataFormatter = useMemo(() => {
    ...
    return (values) => {
      ...
      const deriveBaseSettingDefinitionId = (collection) => {
        ...
        const firstDef = firstChildren.find(
          (child) => child?.settingDefinitionId
        )?.settingDefinitionId
      ...
      if (baseRaw.settingInstance.groupSettingCollectionValue) {
        ...
        const resolvedBaseDefinitionId =
          baseRaw.settingDefinitionId ||
          deriveBaseSettingDefinitionId(normalizedWithIds)
      ...
      return {
        ...
        tenantFilter:
          processedValues.tenantFilter || userSettingsDefaults.currentTenant,
      }
    }
