...
namespace CIPP.Tests
{
    ...
    /// Port of Invoke-CippTestZTNA21984. Skipped on no DirectoryRecommendations data; Passed when no
    /// recommendation has status 'active' and priority 'low'.
    /// </summary>
    public sealed class ZTNA21984 : ICippTest
    {
        public string Id => "ZTNA21984";
        ...
        public CippTestResult Evaluate(TenantData data, ILogSink log)
        {
            ...
            foreach (var r in Items(recs))
                if (StrEq(r, "status", "active") && StrEq(r, "priority", "low")) active.Add(r);

            if (active.Count == 0)
                return new CippTestResult(TestStatus.Passed,
                    "No active low-priority Microsoft Entra recommendations were found.");

            var lines = new List<string>
            {
                $"{active.Count} active low-priority Microsoft Entra recommendation(s) found.",
                "",
