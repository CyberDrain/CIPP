...
const Page = () => {
  ...
  const dateApiData = useMemo(
    () => ({
      ...(dateParams.RelativeTime
        ? { RelativeTime: dateParams.RelativeTime }
        : {}),
  ...
  const kpis = useMemo(() => {
    ...
    return [
      ...
      },
      {
        icon: <ClockIcon />,
        data:
          stats.medianLatency != null
            ? `${Math.round(stats.medianLatency)}m`
            : '—',
        name: 'Median latency',
        color: 'secondary',
      ...
      },
      {
        ...
        color:
          stats.throttleEvents > 0 || stats.retriedWindows > 0
            ? 'warning'
            : 'secondary',
  ...
  const statusChart = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      if (st !== 'Processed') {
        ...
        const t = (r.Tenant || r.TenantId || '?').replace(
          /\.onmicrosoft\.com$/,
          ''
        )
    ...
    const labels = Array.from({ length: b.count }, (_, i) =>
      bucketLabel(bucketStartMs(i, b))
    )
  ...
  const trendCharts = useMemo(() => {
    ...
    const avg = (si) =>
      sums[si].map((s, i) => (cnts[i] ? Math.round(s / cnts[i]) : null))
    const axis = {
      xaxis: {
        categories: labels,
    ...
    return {
      ...
      latOptions: {
        ...
        tooltip: {
          ...
          y: { formatter: (v) => (v == null ? '—' : `${Math.round(v)} min`) },
        },
      },
      volSeries: [
  ...
  const offCanvas = {
    children: (row) => (
      <CippJsonView
        object={row}
        defaultOpen={true}
        title="Coverage Window Details"
      />
    ),
    size: 'lg',
  }

  const tableFilters = tableFilterTenant
    ? [{ id: 'Tenant', value: tableFilterTenant }]
    : []
          ...
          <Stack spacing={2}>
            ...
            <Grid container spacing={2}>
              <Grid size={{ xs: 12, md: 6 }}>
                ...
                >
                  {trendCharts && (
                    <Chart
                      options={trendCharts.latOptions}
                      series={trendCharts.latSeries}
                      type="area"
                      height={300}
                    />
                  )}
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                ...
                >
                  {trendCharts && (
                    <Chart
                      options={trendCharts.volOptions}
                      series={trendCharts.volSeries}
                      type="line"
                      height={300}
                    />
                  )}
                      ...
                      const badge =
                        r.State === 'DeadLetter'
                          ...
                          : r.State === 'Skipped'
                            ...
                            : num(r.ThrottleCount) > 0
                              ? {
                                  label: `Throttle ×${num(r.ThrottleCount)}`,
                                  color: 'warning',
                                }
                              : {
                                  label: `Retry ×${num(r.RetryCount)}`,
                                  color: 'warning',
                                }
                      const when =
                        r.LastErrorUtc || r.ProcessedUtc || r.WindowStart
                      return (
                        <Stack
            ...
            <Card>
              ...
              {tableFilterTenant && (
                ...
                >
                  <Typography variant="body2" color="text.secondary">
                    Filtered to{' '}
                    {tableFilterTenant.replace(/\.onmicrosoft\.com$/, '')}
                  </Typography>
                  <Button
                    size="small"
                    onClick={() => setTableFilterTenant(null)}
                  >
                    Clear
                  </Button>
                </Stack>
              ...
              <CippDataTable
                ...
                title={
                  tenant ? `Search Coverage - ${tenant}` : 'Search Coverage'
                }
                api={{
                  url: '/api/ListAuditLogCoverage',
                  data: { tenantFilter: tenant, ...dateApiData },
