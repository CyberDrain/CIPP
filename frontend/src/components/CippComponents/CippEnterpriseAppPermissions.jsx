...
const lookupDelegatedRiskByScopeName = (scopeName, apiDisplayHint) => {
  ...
  return (
    matches.find((r) => apiLabelsMatch(apiDisplayHint, r.api)) || matches[0]
  )
}
...
const PermissionLine = ({ primary, secondary, riskMeta }) => {
...
const CippEnterpriseAppPermissions = ({ servicePrincipalId, tenantFilter }) => {
  ...
  const grants =
    permBulkRows.find((r) => r.id === 'oauth2Grants')?.body?.value || []
  ...
  const resolveDelegatedScope = (resourceId, scopeValue) => {
    const sp = resourceSpMap[resourceId]
    const scopes =
      sp?.oauth2PermissionScopes || sp?.publishedPermissionScopes || []
  ...
  const sortedResourceIdsApp = useMemo(() => {
    return Object.keys(assignmentsByResource).sort((a, b) => {
      const nameA =
        resourceSpMap[a]?.displayName ||
        assignmentsByResource[a][0]?.resourceDisplayName ||
        a
      const nameB =
        resourceSpMap[b]?.displayName ||
        assignmentsByResource[b][0]?.resourceDisplayName ||
        b
    ...
    <Stack spacing={4} sx={{ py: 2 }}>
      <Box>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          App roles assigned to this enterprise application (app-only), grouped
          by resource API.
        </Typography>
        {permLoading && <Skeleton variant="rectangular" height={80} />}
        {!permLoading && sortedResourceIdsApp.length === 0 && (
          <Alert severity="info">
            No application permissions are assigned.
          </Alert>
        )}
        {!permLoading &&
              ...
              >
                ...
                {!resLoading && (
                  <List dense disablePadding>
                    {list.map((a, idx) => {
                      const { name, description } = resolveAppRole(
                        rid,
                        a.appRoleId
                      )
      ...
      <Box>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          OAuth2 delegated permission grants for this enterprise application,
          grouped by resource API.
        </Typography>
        {permLoading && <Skeleton variant="rectangular" height={80} />}
        {!permLoading && sortedResourceIdsDelegated.length === 0 && (
          <Alert severity="info">
            No delegated permission grants are present.
          </Alert>
        )}
        {!permLoading &&
                ...
                {!resLoading && (
                  <List dense disablePadding>
                    {scopeList.map((scopeName, idx) => {
                      const { description } = resolveDelegatedScope(
                        rid,
                        scopeName
                      )
                      const riskMeta = lookupDelegatedRiskByScopeName(
                        scopeName,
                        title
                      )
                      return (
