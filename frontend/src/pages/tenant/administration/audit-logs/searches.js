  ...
  const health = useMemo(() => {
    ...
    const rows = Array.isArray(d)
      ...
      : Array.isArray(d?.Results)
        ? d.Results
        : []
    const searching = rows.filter(
      (r) => r.State === 'Planned' || r.State === 'Created'
    ).length
        ...
        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
          {health.failed === 0 ? (
            <Chip
              size="small"
              color="success"
              label="All log searches healthy"
            />
          ) : (
            <Chip
              size="small"
              color="error"
              label={`${health.failed} failed permanently`}
            />
          )}
          <Chip
            size="small"
          ...
          {health.skipped > 0 && (
            <Chip
              size="small"
              variant="outlined"
              label={`${health.skipped} skipped (auditing off)`}
            />
          )}
        </Stack>
      )}
