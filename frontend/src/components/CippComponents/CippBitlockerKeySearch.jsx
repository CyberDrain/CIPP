    ...
    try {
      ...
      if (response?.data?.Results?.copyField) {
        setRecoveryKeys((prev) => ({
          ...prev,
          [keyId]: response.data.Results.copyField,
        }))
      }
    } catch (error) {
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      {recoveryKeys[result.keyId] ? (
                        <>
                          ...
                          <CippCopyToClipBoard
                            text={recoveryKeys[result.keyId]}
                          />
                        </>
                      ) : (
                        <Button
                  ...
                  {!result.deviceFound && (
                    <Grid size={{ xs: 12 }}>
                      <Alert severity="warning" icon={<Info />}>
                        Device information not found in cache. The device may
                        have been deleted or not yet synced.
                      </Alert>
                    </Grid>
                  )}
                </Grid>
