      ...
      <TableBody>
        {cases.map(({ cellName, data, canReceive, description }, i) => (
          <TableRow key={`${cellName}-${i}`}>
            ...
            <TableCell>
              {String(getCippFormatting(data, cellName, 'text', canReceive))}
            </TableCell>
            <TableCell>
              <Box
                sx={{
...
export const SpecialFields = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      'fixed labels for addrow, baseline, delegated privilege',
      async () => {
...
export const SeverityAndStatus = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('status chips carry semantic colors', async () => {
      expect(
        canvasElement.querySelector('.MuiChip-colorSuccess')
      ).not.toBeNull()
      expect(canvasElement.querySelector('.MuiChip-colorError')).not.toBeNull()
      expect(
        canvasElement.querySelector('.MuiChip-colorWarning')
      ).not.toBeNull()
...
export const StateField = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('state values normalize to display labels', async () => {
      ...
      expect(canvas.getAllByText('Report Only').length).toBeGreaterThanOrEqual(
        4
      )
    })
  },
...
export const Booleans = {
  ...
  play: async ({ canvasElement, step }) => {
    await step(
      'booleans render Yes/No text and check/cancel icons',
      async () => {
        await expect(canvasElement.textContent).toContain('Yes')
        await expect(canvasElement.textContent).toContain('No')
        expect(
          canvasElement.querySelector('[data-testid="CheckIcon"]')
        ).not.toBeNull()
        expect(
          canvasElement.querySelector('[data-testid="CancelIcon"]')
        ).not.toBeNull()
      }
    )
...
export const DateTimeFields = {
  ...
  play: async ({ canvasElement, step }) => {
    await step(
      'date fields render relative time in component mode',
      async () => {
...
export const PortalLinks = {
  ...
  play: async ({ canvasElement, step }) => {
    await step(
      'each portal renders a new-tab icon link with its url',
      async () => {
        expect(
          canvasElement.querySelectorAll('a[target="_blank"]')
        ).toHaveLength(6)
        expect(
          canvasElement.querySelector('a[href="https://entra.microsoft.com"]')
        ).not.toBeNull()
      }
    )
  },
...
export const BytesAndPercentages = {
  ...
  play: async ({ canvasElement, step }) => {
    await step(
      'bytes convert to two-decimal GB, null falls back to No data',
      async () => {
...
export const JSONStrings = {
  ...
  play: async ({ canvasElement, step }) => {
    await step(
      'json strings parse to joined arrays, booleans, quoted singles',
      async () => {
        await expect(canvasElement.textContent).toContain('tag1, tag2, tag3')
        await expect(canvasElement.textContent).toContain('Yes')
...
export const MiscFields = {
  ...
  play: async ({ canvasElement, step }) => {
    await step(
      'long hashes truncate at 15 chars, bulk users count',
      async () => {
        await expect(canvasElement.textContent).toContain('ABCDEFGHIJKLMNO...')
        await expect(canvasElement.textContent).toContain(
          '5 new users to create'
        )
      }
    )
    ...
    await step('standard types, translation keys, html and links', async () => {
      ...
      expect(
        canvasElement.querySelector('a[href="https://example.com/path"]')
      ).not.toBeNull()
    })
  },
}
...
export const ArrayOfJSONStrings = {
  ...
  play: async ({ canvasElement, step }) => {
    await step(
      'quoted strings join unparsed, broken json falls back to raw join',
      async () => {
