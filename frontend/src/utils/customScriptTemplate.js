const parsePathSegments = (expression) => {
  ...
  while (cursor < remainder.length) {
    ...
    if (char === '[') {
      ...
      segments.push(
        bracketMatch[1] === '*' ? '*' : Number.parseInt(bracketMatch[1], 10)
      )
...
const resolveToken = (result, tokenExpression) => {
  ...
  if (fallbackMatch) {
    ...
    return baseValue === ''
      ? decodeEscapedTemplateString(fallbackMatch[3])
      : baseValue
  }
  ...
  if (joinMatch) {
    ...
    const values = Array.isArray(joinedValue)
      ? joinedValue
      : joinedValue == null
        ? []
        : [joinedValue]
    return values
      ...
      .join(decodeEscapedTemplateString(joinMatch[3]))
  }
