    ...
    if (bpaTemplateList.isSuccess) {
      ...
      if (bpaTemplate) {
        ...
        if (bpaTemplate.Style === 'Tenant') {
          const frontendFields = bpaTemplate.Data.map(
            (block) => block.FrontendFields[0]
          )
          if (bpaData.isSuccess) {
            ...
            const tenantData = bpaData?.data?.Data?.find(
              (data) => data.GUID === tenantId
            )
          ...
          if (bpaData.isSuccess) {
            ...
            if (bpaData.isSuccess) {
              ...
              const listOfFrontEndFields = flatFrontendFields.map((subField) =>
                ...
                subField?.value?.includes(' ')
                  ? subField.value.split(' ')[0]
                  : subField.value
              )
                  ...
                  >
                    ...
                    >
                      {block.data === undefined ? (
                        <Alert severity="info">
                          No data has been found for this item. This tenant
                          might not be licensed for this feature, or data
                          collection failed. Please check the logs for more
                          information.
                        </Alert>
                        ...
                        <Typography variant="h6" color="textPrimary">
                          Something is wrong with your report. This field is not
                          formatted correctly.
                        </Typography>
                      )}
                    </CippButtonCard>
