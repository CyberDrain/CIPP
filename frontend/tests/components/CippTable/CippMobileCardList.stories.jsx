...
const actions = [
  {
    label: 'Edit user',
    icon: <Edit />,
    link: '/identity/administration/users/edit?id=[id]',
  },
  {
    label: 'Block sign-in',
    icon: <Block />,
...
export const Default = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      'row kebab opens the action sheet with the page actions',
      async () => {
        const kebabs = canvas.getAllByRole('button', { name: /row actions/i })
        await userEvent.click(kebabs[0])
        const body = within(document.body)
        await waitFor(() =>
          expect(body.getByText('Block sign-in')).toBeInTheDocument()
        )
    ...
    await step(
      'Filters opens the shared bottom sheet with the card fields',
      async () => {
        const body = within(document.body)
        await userEvent.click(
          canvas.getByRole('button', { name: 'Table options' })
        )
        const filterSheet = await body.findByRole('dialog')
        expect(
          within(filterSheet).getByText('Fields shown')
        ).toBeInTheDocument()
        await userEvent.keyboard('{Escape}')
        await waitFor(() => expect(body.queryByRole('dialog')).toBeNull())
      }
...
export const SelectMode = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      'Select reveals per-card checkboxes and the bulk bar',
      async () => {
        await userEvent.click(canvas.getByRole('button', { name: /select/i }))
        const checkboxes = await canvas.findAllByRole('checkbox')
        await userEvent.click(checkboxes[0])
        await waitFor(() =>
          expect(canvasElement.textContent).toContain('1 selected')
        )
      }
    )
  },
...
export const PageActionsFab = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('cardButton children live behind the FAB', async () => {
      ...
      await waitFor(() =>
        expect(
          body.getByRole('button', { name: 'Add User' })
        ).toBeInTheDocument()
      )
...
export const LoadMore = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('starts at the configured page size', async () => {
      await waitFor(
        () => expect(canvasElement.textContent).toContain('Showing 25 of 120'),
        {
          timeout: 10000,
        }
      )
    ...
    await step('Load more grows the same list rather than paging', async () => {
      await userEvent.click(
        canvas.getByRole('button', { name: /load 50 more/i })
      )
      await waitFor(() =>
        expect(canvasElement.textContent).toContain('Showing 75 of 120')
      )
      // still one continuous list — no pagination control appeared
      expect(
        canvas.queryByRole('button', { name: /go to next page/i })
      ).toBeNull()
    })
  },
}
...
const applyDepartmentSearch = async (canvas) => {
  ...
  await waitFor(() => expect(canvas.queryByText('Bob Johnson')).toBeNull(), {
    timeout: 5000,
  })
...
export const MobileCards = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      'card view yields the identical row set from the same state',
      async () => {
        expect(canvasElement.querySelector('table')).toBeNull()
        const matched = await applyDepartmentSearch(canvas)
        expect(matched).toHaveLength(2)
...
export const TableViewToggle = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() =>
      expect(canvas.getByTestId('cipp-mobile-card-list')).toBeInTheDocument()
    )

    await userEvent.click(
      await canvas.findByRole('button', { name: 'Toggle table view' })
    )
    await waitFor(() => {
      expect(canvasElement.querySelector('table')).not.toBeNull()
    ...
    expect(
      canvasElement.querySelector('[aria-label="Refresh data"]')
    ).toBeNull()
    ...
    expect(
      within(filterSheet).getByText('Reset all filters')
    ).toBeInTheDocument()
    ...
    const activeSize = within(filterSheet)
      .getByText('25')
    ...
    await userEvent.click(
      canvas.getByRole('button', { name: 'Toggle table view' })
    )
    await waitFor(() =>
      expect(canvas.getByTestId('cipp-mobile-card-list')).toBeInTheDocument()
    )
...
export const TableViewToggleWithActions = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() =>
      expect(body.getByRole('button', { name: 'Add User' })).toBeInTheDocument()
    )
  },
}
...
export const TableViewToggleBulkActionsInHeader = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() => {
      ...
      expect(
        within(header).getByRole('button', { name: 'Bulk Actions' })
      ).toBeInTheDocument()
    })
    // exactly one Bulk Actions button on screen — it moved, it did not duplicate
    expect(
      canvas.getAllByRole('button', { name: 'Bulk Actions' })
    ).toHaveLength(1)
...
export const DesktopBulkActionsStayInToolbar = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const header = canvasElement.querySelector('.MuiCardHeader-root')
    expect(
      within(header).queryByRole('button', { name: 'Bulk Actions' })
    ).toBeNull()
  },
}
