...
const renderReport = () =>
  renderWithProviders(
    <CippPermissionReport importReport={false} setImportReport={() => {}} />
  )
...
describe('CippPermissionReport report actions', () => {
  ...
  it('moves the buttons into the FAB sheet on mobile', async () => {
    ...
    expect(
      screen.getByRole('button', { name: /paste report/i })
    ).toBeInTheDocument()

    // uniform with every other sheet action: list rows, not contained buttons in a sheet
    expect(
      document.querySelector('.MuiDrawer-paper .MuiButton-contained')
    ).toBeNull()
    expect(
      screen
        .getByRole('button', { name: /export report/i })
        .classList.contains('MuiListItemButton-root')
    ).toBe(true)
  })
  ...
  it('closes the sheet when a row opens its dialog', async () => {
    ...
    const exportRow = await screen.findByRole('button', {
    ...
    await vi.waitFor(() =>
      expect(screen.getByText(/paste report/i)).not.toBeVisible()
    )
  })
})
